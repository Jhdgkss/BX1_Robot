"""
Robot Brain V2.10.0 - Brain-Owned Wake and Body Voice Edition
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "app_version": "Robot Brain V2.10.0 - Brain-Owned Wake and Local Body Voice",
    "robot_name": "BX1",
    ...
    def _dottts_extra(self, extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        options: Dict[str, Any] = {
            **selected,
            ...
            "profile": str(PROFILE_NAME or "bx1"),
        }
        if extra:
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.save_header_button = QPushButton("Save")
        self.save_header_button.setObjectName("PrimaryButton")
        self.save_header_button.setToolTip("Save all current settings")
        ...
        nav_items = [
            ("Overview", "Chat and system status"),
            ("Robot", "Identity, voice and tools"),
            ("Knowledge", "Documents and memory"),
            ("Behaviours", "Design safe capabilities"),
            ("Hardware", "Telemetry, camera and actions"),
        ...
        self.statusBar().showMessage(f"Active robot: {robot_name_from_cfg(self.cfg)}   •   Profile: {PROFILE_NAME or 'default'}")
    ...
    def _build_home_workspace(self) -> QWidget:
        ...
        grid.setSpacing(10)
        welcome = QLabel(f"Welcome to {robot_name_from_cfg(self.cfg)}\nEverything you need, at a glance")
        ...
        cards = [
            ...
            ("VOICE", "Speech engine and voice profile"),
            ("CAMERA", "Latest robot camera frame"),
            ("WORKSHOP", "Safe behaviour design workspace"),
        ]
        ...
        quick = QGroupBox("Quick setup")
        quick_row = QHBoxLayout(quick)
        identity_button = QPushButton("Name & Personality")
        identity_button.clicked.connect(self.open_identity_editor)
        voice_button = QPushButton("Voice Setup")
        voice_button.clicked.connect(self.open_voice_setup)
        profile_button = QPushButton("Robot Profiles")
        profile_button.clicked.connect(self.open_profile_manager)
        diagnostic_button = QPushButton("Run Diagnostics")
        diagnostic_button.clicked.connect(lambda: self.switch_workspace(5))
        for button in (identity_button, voice_button, profile_button, diagnostic_button):
            quick_row.addWidget(button)
        grid.addWidget(quick, 4, 0, 1, 2)
        note = QLabel("Tip: start with Name & Personality, then Voice Setup. Hardware safety and motor limits remain on the robot body.")
    ...
    def _build_memory_overview_panel(self) -> QWidget:
        ...
        text = QLabel("Conversation memory and profile-owned knowledge remain local. Detailed controls are available in Robot → Voice and Knowledge → Documents / RAG.")
    ...
    def _build_identity_summary_panel(self) -> QWidget:
        ...
        title.setObjectName("SectionTitle")
        text = QLabel(
            "The robot name and configuration profile are separate. The profile selects isolated config, runtime, documents and voice files. "
        ...
        details.setPlainText(
            ...
            "Use Robot Profiles in the header to create or select another robot.\n"
            f"Manual command:\n.venv\\Scripts\\python.exe main_pyqt.py --profile {profile}"
        )
        buttons = QHBoxLayout()
        button = QPushButton("Open Identity Editor")
        button.setObjectName("PrimaryButton")
        button.clicked.connect(self.open_identity_editor)
    ...
    def _build_identity_tab(self) -> QWidget:
        ...
        personality_library_hint = QLabel(
            "Save several characters for this robot and switch between them without changing its hardware, "
            "connections, documents or memory. A personality can remember its character name and chosen voice profile."
        )
    ...
    def refresh_personality_library_ui(self, preferred_slug: str = "") -> None:
        ...
        self.active_personality_slug = self.personality_store.selected_slug()
        try:
    ...
    def _apply_personality_cfg_to_widgets(self) -> None:
        if not hasattr(self, "robot_name_edit"):
            return
    ...
    def save_personality_as_new_ui(self) -> None:
        ...
        try:
            ...
            self.active_personality_slug = profile.slug
            self.cfg["selected_personality_profile"] = profile.slug
            save_config(self.cfg)
            self.refresh_personality_library_ui(profile.slug)
    ...
    def _build_help_tab(self) -> QWidget:
        ...
        <p>Use <b>Saved personality library</b> to load, save, duplicate, rename or delete characters inside the active robot profile. Character changes do not alter hardware settings, memory, documents or the Dot.TTS service.</p>
        <p><b>Current Personality</b> is imported from the settings already in use. <b>Curious Female Companion</b> is supplied as a second saved choice. Her accepted self-chosen name is stored with that personality and returns when it is loaded again.</p>
    ...
    def refresh_cfg_from_widgets(self) -> None:
        ...
        if hasattr(self, "web_enabled_check"):
            self.cfg.update({
                ...
                "google_search_api_key": getattr(self, "google_api_key_edit", None).text().strip() if hasattr(self, "google_api_key_edit") else str(self.cfg.get("google_search_api_key", "")),
                "google_search_cx": getattr(self, "google_cx_edit", None).text().strip() if hasattr(self, "google_cx_edit") else str(self.cfg.get("google_search_cx", "")),
                "aviation_weather_enabled": bool(getattr(self, "aviation_weather_check", self.weather_enabled_check).isChecked()),
                "aviation_default_airport": (getattr(self, "aviation_airport_edit", None).text().strip().upper() if hasattr(self, "aviation_airport_edit") else str(self.cfg.get("aviation_default_airport", "EGCB"))) or "EGCB",
                "metoffice_enabled": bool(getattr(self, "metoffice_enabled_check", self.weather_enabled_check).isChecked()),
                "metoffice_api_key": getattr(self, "metoffice_api_key_edit", None).text().strip() if hasattr(self, "metoffice_api_key_edit") else str(self.cfg.get("metoffice_api_key", "")),
                "metoffice_spot_url": getattr(self, "metoffice_url_edit", None).text().strip() if hasattr(self, "metoffice_url_edit") else str(self.cfg.get("metoffice_spot_url", "")),
                "vision_overlay_enabled": bool(getattr(self, "vision_overlay_check", None).isChecked()) if hasattr(self, "vision_overlay_check") else bool(self.cfg.get("vision_overlay_enabled", True)),
            })
        if hasattr(self, "voice_enabled_check"):
            self.cfg.update({
                ...
                "voice_engine": self._combo_text(self.voice_engine_edit, "dottts"),
                ...
                "dottts_service_url": self.dottts_service_url_edit.text().strip().rstrip("/") or "http://127.0.0.1:8092",
                "dottts_model": self._combo_text(self.dottts_model_edit, "mf"),
                "dottts_auto_start": bool(self.dottts_auto_start_check.isChecked()),
                "dottts_stop_with_app": bool(self.dottts_stop_check.isChecked()),
                "dottts_warmup_on_start": bool(self.dottts_warmup_check.isChecked()),
                "dottts_play_on_brain_pc": bool(self.dottts_play_check.isChecked()),
                "dottts_sampling_steps": int(self.dottts_steps_spin.value()),
                "dottts_guidance_scale": float(self.dottts_guidance_spin.value()),
                "dottts_emotional_delivery_enabled": bool(self.dottts_emotional_check.isChecked()),
                "dottts_inline_delivery_instructions": bool(self.dottts_inline_emotion_check.isChecked()),
                "dottts_default_delivery": self._combo_text(self.dottts_default_delivery_combo, "normal"),
    ...
    def open_dottts_lab_ui(self) -> None:
        self.refresh_cfg_from_widgets()
        profile = quote_plus(str(PROFILE_NAME or "bx1"))
        lab_url = self._dottts_base_url() + f"/lab?profile={profile}"
        ...
        self.append_voice_status("Opened the Dot.TTS English Voice Lab.")
