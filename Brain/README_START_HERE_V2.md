# Robot Brain V2.10.0 — Start Here

V2.10.0 adds a Brain-owned wake and queued-speech profile for the physical UNO Q robot. Open **Robot → Body Wake / Queued Speech** to define wake templates, optional aliases, wake acknowledgements, waiting phrases and the sleep acknowledgement. Press **Generate All** to render the phrases with the active voice; BX1 Body v10.40 synchronises and stores them locally.

The robot name comes from **Robot → Identity / Personality → Robot name**. Wake templates such as `hey {robot_name}` automatically follow that name.
