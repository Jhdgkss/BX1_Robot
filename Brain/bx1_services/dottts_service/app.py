...
<p class="hint"><strong id="profileLabel">Personality voice</strong></p>
...
const profile=params.get('profile')||'bx1';
const profileLabel=params.get('label')||profile;
$('profileLabel').textContent=`Personality voice: ${profileLabel}`;
document.title=`Dot.TTS Voice Lab - ${profileLabel}`;
...
def main() -> None:
    ...
    parser.add_argument("--model", choices=["mf", "soar"], default="mf")
    parser.add_argument("--precision", default="bfloat16")
    parser.add_argument("--optimize", action="store_true", help="Enable optional torch.compile optimisation; CUDA toolkit/nvcc is required")
    ...
    Handler.engine = DotEngine(model, args.precision, optimize)
    mode = "optimized (CUDA toolkit/nvcc required)" if optimize else "standard compatibility mode (torch.compile disabled)"
