...
class BX1BrainClient:
    ...
    def body_profile(self) -> Dict[str, Any]:
        """Read Brain-owned identity, wake phrases and queued body speech."""
        return self._get_json("/api/body_profile", timeout=15)

    def generate_body_audio(self, keys: Optional[List[str]] = None) -> Dict[str, Any]:
        """Ask the Brain app to generate selected/all queued body phrases."""
        return self._post_json("/api/body_audio/generate", {"keys": list(keys or [])}, timeout=max(1200, self.cfg.chat_timeout_s))

    def download_body_audio(self, audio_url: str, target: Path, timeout_s: int = 120) -> Path:
        """Atomically download one Brain-generated phrase into the UNO Q cache."""
        raw_url = str(audio_url or "").strip()
        if not raw_url:
            raise RuntimeError("Body phrase did not include an audio URL.")
        ...
        try:
            with requests.get(url, headers=self._headers(), stream=True, timeout=max(10, int(timeout_s))) as response:
            ...
            if temporary.stat().st_size < 256:
                raise RuntimeError("Downloaded body phrase audio is too small to be valid.")
