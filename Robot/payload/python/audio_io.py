...
class AlsaMicrophoneMonitor:
    """Single ALSA owner shared by wake detection, STT and diagnostics.

    The UNO Q USB microphone is opened once. Consumers subscribe to the rolling
    20 ms PCM stream instead of starting competing ``arecord`` processes. This
    removes the historical "microphone is busy" handover race and ensures every
    capture has a deterministic timeout.
    """
    ...
    def __init__(self, device: str = "default", sample_rate: int = 16000, channels: int = 1,
        ...
        self._frame_seq = 0
        # Thirty seconds is enough for wake pre-roll, diagnostics and transient
        # inspection without creating a meaningful memory burden.
        self._frames: deque[dict[str, Any]] = deque(maxlen=max(300, int(30000 / self.frame_ms)))
        self._meter_enabled = True
        self._stats: dict[str, Any] = {
            "running": False, "stream_running": False, "meter_enabled": True,
            "stream_owner": "bx1_shared_audio", "device": self.device,
            ...
            "frame_sequence": 0, "buffer_seconds": 0.0,
        }
    ...
    def update_settings(self, device: str, sample_rate: int, channels: int = 1,
        ...
        meter_enabled = self._meter_enabled
        if was_running:
            self.stop()
        self.device = str(device or "default")
        self.sample_rate = int(max(8000, min(48000, sample_rate)))
        ...
        with self._lock:
            self._stats.update({
                ...
                "raw_rms_dbfs": -120.0, "raw_peak_dbfs": -120.0,
                "rms_dbfs": -120.0, "peak_dbfs": -120.0, "rms_pct": 0.0, "peak_pct": 0.0,
                "active": False, "clipped": False, "error": "", "spectrum_raw": [],
                "spectrum_filtered": [], "frame_sequence": 0, "buffer_seconds": 0.0,
            })
    ...
    def set_meter_enabled(self, enabled: bool) -> dict[str, Any]:
        self._meter_enabled = bool(enabled)
        with self._lock:
            self._stats["meter_enabled"] = self._meter_enabled
        if not self.is_running():
            self.start()
        return {
            "ok": True,
            "message": "microphone level display enabled" if enabled else "microphone level display stopped; shared audio remains active",
            "level": self.snapshot(),
        }
    ...
    def start(self) -> dict:
        self._meter_enabled = True
        if self.is_running():
            return {"ok": True, "message": "shared microphone stream already running", "level": self.snapshot()}
        if not shutil.which("arecord"):
            return {"ok": False, "error": "arecord not found"}
        self._stop.clear()
        self._thread = threading.Thread(target=self._run, name="bx1-shared-audio", daemon=True)
        self._thread.start()
        return {"ok": True, "message": "shared microphone stream started", "level": self.snapshot()}

    def ensure_running(self, timeout_s: float = 3.0) -> dict[str, Any]:
        if not self.is_running():
            result = self.start()
            if not result.get("ok"):
                return result
        deadline = time.monotonic() + max(0.2, float(timeout_s))
        initial = self.latest_sequence()
        while time.monotonic() < deadline:
            if self.latest_sequence() > initial or self.snapshot().get("last_update"):
                return {"ok": True, "level": self.snapshot()}
            if self.snapshot().get("error"):
                return {"ok": False, "error": self.snapshot().get("error")}
            time.sleep(0.03)
        return {"ok": False, "error": "shared microphone stream did not produce audio frames before timeout"}
    ...
    def stop(self) -> dict:
        ...
        with self._lock:
            ...
            self._stats["stream_running"] = False
        return {"ok": True, "message": "shared microphone stream stopped", "level": self.snapshot()}

    def snapshot(self) -> dict:
        with self._lock:
            snap = dict(self._stats)
        stream_running = self.is_running()
        snap["stream_running"] = stream_running
        snap["running"] = bool(stream_running and self._meter_enabled)
        snap["meter_enabled"] = bool(self._meter_enabled)
    ...
    def recent_frames(self, milliseconds: int) -> list[dict[str, Any]]:
        count = max(0, int(math.ceil(max(0, int(milliseconds)) / float(self.frame_ms))))
        with self._condition:
            frames = list(self._frames)
        return frames[-count:] if count else []

    def wait_for_frames(self, after_sequence: int, timeout_s: float = 0.25) -> list[dict[str, Any]]:
        deadline = time.monotonic() + max(0.01, float(timeout_s))
        with self._condition:
            while not self._stop.is_set() and self._frame_seq <= int(after_sequence):
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    return []
                self._condition.wait(timeout=min(0.10, remaining))
            return [dict(frame) for frame in self._frames if int(frame.get("seq", 0)) > int(after_sequence)]

    def capture_fixed(self, seconds: float, cancel_event: Optional[threading.Event] = None) -> dict[str, Any]:
        ready = self.ensure_running()
        if not ready.get("ok"):
            return {"ok": False, "error": ready.get("error", "microphone stream unavailable")}
        duration = max(0.1, min(120.0, float(seconds or 1.0)))
        ...
        while time.monotonic() - started < duration:
            if cancel_event is not None and cancel_event.is_set():
                return {"ok": True, "cancelled": True, "reason": "capture cancelled", "duration_s": 0.0}
            frames = self.wait_for_frames(sequence, timeout_s=min(0.25, duration))
            for frame in frames:
                sequence = max(sequence, int(frame.get("seq", sequence)))
                raw_parts.append(bytes(frame.get("raw") or b""))
                clean_parts.append(bytes(frame.get("clean") or b""))
        raw_pcm = b"".join(raw_parts)
        clean_pcm = b"".join(clean_parts)
        return {
            "ok": bool(clean_pcm), "speech_started": bool(clean_pcm), "close_reason": "fixed capture window",
            "duration_s": round(len(clean_pcm) / float(self.sample_rate * 2), 3) if clean_pcm else 0.0,
            "sample_rate": self.sample_rate, "channels": 1, "capture_method": "shared_stream_fixed",
            "_raw_pcm": raw_pcm, "_clean_pcm": clean_pcm,
        }

    def capture_utterance(
        self, *, start_timeout_s: float = 8.0, max_utterance_s: float = 20.0,
        pre_roll_ms: int = 650, end_silence_ms: int = 600, post_roll_ms: int = 160,
        start_trigger_ms: int = 100, speech_resume_trigger_ms: int = 140,
        transient_guard_after_ms: int = 220, endpoint_hysteresis_db: float = 3.0,
        fixed_gate_dbfs: float = -45.0, adaptive_threshold_enabled: bool = True,
        adaptive_margin_db: float = 4.0, cancel_event: Optional[threading.Event] = None,
        force_start: bool = False,
    ) -> dict[str, Any]:
        """Capture one endpointed utterance from the already-open shared stream."""
        ready = self.ensure_running()
        if not ready.get("ok"):
            return {"ok": False, "error": ready.get("error", "microphone stream unavailable"), "reason": "capture failed"}
        ...
        silence_frames_required = max(1, int(math.ceil(max(frame_ms, end_silence_ms) / frame_ms)))
        post_frames = max(0, int(math.ceil(max(0, post_roll_ms) / frame_ms)))
        start_timeout_s = max(0.5, min(60.0, float(start_timeout_s or 8.0)))
        max_utterance_s = max(1.0, min(120.0, float(max_utterance_s or 20.0)))
        fixed_gate = max(-90.0, min(-5.0, float(fixed_gate_dbfs or -45.0)))
        adaptive_margin = max(0.0, min(30.0, float(adaptive_margin_db or 4.0)))
        endpoint_hysteresis = max(0.0, min(12.0, float(endpoint_hysteresis_db or 0.0)))

        recent = self.recent_frames(pre_roll_ms)
        pre: deque[dict[str, Any]] = deque(recent[-pre_frames:], maxlen=pre_frames)
        sequence = self.latest_sequence()
        raw_captured: list[bytes] = []
        clean_captured: list[bytes] = []
        noise_levels: deque[float] = deque(maxlen=max(20, int(2000 / frame_ms)))
        for frame in recent:
            noise_levels.append(float(frame.get("rms_dbfs", -120.0)))
        level_trace: deque[float] = deque(maxlen=250)
        speech_started = bool(force_start)
        started_mono = time.monotonic()
        speech_started_at = started_mono if force_start else 0.0
        start_run = 0
        silence_run = 0
        resume_run = 0
        ignored_transients = 0
        ...
        current_threshold = fixed_gate
        continue_threshold = fixed_gate
        if force_start and noise_levels:
            ordered = sorted(noise_levels)
            noise_floor_seed = ordered[max(0, int((len(ordered) - 1) * 0.20))]
            if adaptive_threshold_enabled:
                current_threshold = max(fixed_gate, min(-12.0, noise_floor_seed + adaptive_margin))
            continue_threshold = max(-90.0, current_threshold - endpoint_hysteresis)
        if force_start:
            for frame in pre:
                raw_captured.append(bytes(frame.get("raw") or b""))
                clean_captured.append(bytes(frame.get("clean") or b""))
            pre.clear()

        while not self._stop.is_set():
            if cancel_event is not None and cancel_event.is_set():
                return {"ok": True, "cancelled": True, "speech_started": False, "reason": "capture cancelled", "close_reason": "cancelled"}
            now = time.monotonic()
            if not speech_started and now - started_mono >= start_timeout_s:
                close_reason = "start timeout"
                break
            if speech_started and now - speech_started_at >= max_utterance_s:
                close_reason = "maximum utterance length"
                break
            frames = self.wait_for_frames(sequence, timeout_s=0.12)
            if not frames:
                continue
            for frame in frames:
                sequence = max(sequence, int(frame.get("seq", sequence)))
                now = time.monotonic()
                level = float(frame.get("rms_dbfs", -120.0))
                peak_dbfs = max(peak_dbfs, float(frame.get("peak_dbfs", level)))
                level_trace.append(round(level, 1))
                if not speech_started:
                    pre.append(frame)
                    noise_levels.append(level)
                    ...
                    if adaptive_threshold_enabled:
                        current_threshold = max(fixed_gate, min(-12.0, noise_floor + adaptive_margin))
                    continue_threshold = max(-90.0, current_threshold - endpoint_hysteresis)
                    start_run = start_run + 1 if level >= current_threshold else 0
                    if start_run >= start_frames_required:
                        speech_started = True
                        speech_started_at = now - ((start_run - 1) * frame_ms / 1000.0)
                        for buffered in pre:
                            raw_captured.append(bytes(buffered.get("raw") or b""))
                            clean_captured.append(bytes(buffered.get("clean") or b""))
                        pre.clear()
                        silence_run = 0
                        resume_run = 0
                    continue
                ...
                if level >= continue_threshold:
                    if silence_run < transient_guard_frames:
                        silence_run = 0; resume_run = 0; post_remaining = 0
                    else:
                        resume_run += 1
                        if resume_run >= resume_frames_required:
                            silence_run = 0; resume_run = 0; post_remaining = 0
        ...
        if speech_started and clean_captured:
            raw_pcm = b"".join(raw_captured)
            clean_pcm = b"".join(clean_captured)
            ordered = sorted(noise_levels)
            noise_floor = ordered[len(ordered) // 2] if ordered else None
            return {
                "ok": True, "speech_started": True, "sample_rate": self.sample_rate, "channels": 1,
                "duration_s": round(len(clean_pcm) / float(self.sample_rate * 2), 3),
                "waited_for_speech_s": round(max(0.0, speech_started_at - started_mono), 3),
                "close_reason": close_reason or "completed", "threshold_dbfs": round(current_threshold, 1),
                "continue_threshold_dbfs": round(continue_threshold, 1),
                "noise_floor_dbfs": round(float(noise_floor), 1) if noise_floor is not None else None,
                "peak_dbfs": round(peak_dbfs, 1), "pre_roll_ms": pre_roll_ms,
                "end_silence_ms": end_silence_ms, "post_roll_ms": post_roll_ms,
                "speech_resume_trigger_ms": speech_resume_trigger_ms,
                "transient_guard_after_ms": transient_guard_after_ms,
                "endpoint_hysteresis_db": endpoint_hysteresis,
                "ignored_transients": ignored_transients, "level_trace_dbfs": list(level_trace),
                "capture_method": "shared_stream_endpointed", "force_start": bool(force_start),
                "_raw_pcm": raw_pcm, "_clean_pcm": clean_pcm,
            }
        return {
            "ok": True, "speech_started": False, "duration_s": 0.0,
            "close_reason": close_reason or "no speech", "reason": "no speech detected before start timeout",
            "threshold_dbfs": round(current_threshold, 1), "continue_threshold_dbfs": round(continue_threshold, 1),
            "peak_dbfs": round(peak_dbfs, 1), "level_trace_dbfs": list(level_trace),
            "capture_method": "shared_stream_endpointed",
        }
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            with self._lock:
                self._stats.update({"running": True, "stream_running": True, "error": "", "command": cmd})
            frame_bytes = max(2, int(self.sample_rate * self.channels * 2 * (self.frame_ms / 1000.0)))
            update_no = 0
            while not self._stop.is_set():
                ...
                raw_level = _pcm16_level(raw_mono, software_gain_db=0.0)
                active = bool(level["rms_dbfs"] >= float(self.noise_gate_dbfs))
                update_no += 1
                extra: dict[str, Any] = {}
                if self._meter_enabled and update_no % 15 == 0:
                ...
                with self._condition:
                    self._frame_seq += 1
                    frame = {
                        "seq": self._frame_seq, "raw": raw_mono, "clean": clean,
                        "rms_dbfs": float(level.get("rms_dbfs", -120.0)),
                        "peak_dbfs": float(level.get("peak_dbfs", -120.0)),
                        "raw_rms_dbfs": float(raw_level.get("rms_dbfs", -120.0)),
                        "raw_peak_dbfs": float(raw_level.get("peak_dbfs", -120.0)),
                        "timestamp_mono": time.monotonic(),
                    }
                    self._frames.append(frame)
                    self._condition.notify_all()
                    frame_seq = self._frame_seq
                    buffer_seconds = len(self._frames) * self.frame_ms / 1000.0
                with self._lock:
                    self._stats.update(level)
                    self._stats["raw_rms_dbfs"] = raw_level.get("rms_dbfs", -120.0)
                    self._stats["raw_peak_dbfs"] = raw_level.get("peak_dbfs", -120.0)
                    ...
                    self._stats["stream_running"] = True
                    self._stats["meter_enabled"] = self._meter_enabled
                    self._stats["frame_sequence"] = frame_seq
                    self._stats["buffer_seconds"] = round(buffer_seconds, 2)
            err = ""
            try:
...
class VoskSpeechToText:
    ...
    def wait_for_wake_phrase(
        self, audio_source: Any, wake_phrases: list[str], *, timeout_s: float = 2.0,
        stop_event: Optional[threading.Event] = None, sensitivity: float = 0.72,
    ) -> dict[str, Any]:
        """Run lightweight streaming Vosk grammar recognition on shared PCM.

        This is keyword spotting, not full transcription. It never opens ALSA and
        therefore can run alongside the level meter and diagnostic subscribers.
        """
        phrases = [" ".join(str(item or "").lower().split()) for item in wake_phrases]
        phrases = list(dict.fromkeys(item for item in phrases if item))
        if not phrases:
            return {"ok": False, "detected": False, "reason": "no wake phrases configured"}
        if self.KaldiRecognizer is None or self.model is None:
            return {"ok": False, "detected": False, "reason": "local Vosk model is not ready", "error": self.error}
        if audio_source is None or not hasattr(audio_source, "wait_for_frames"):
            return {"ok": False, "detected": False, "reason": "shared audio source unavailable"}
        try:
            grammar = json.dumps([*phrases, "[unk]"])
            rec = self.KaldiRecognizer(self.model, int(getattr(audio_source, "sample_rate", self.cfg.sample_rate)), grammar)
        except Exception:
            # Older Vosk builds may not accept grammar in the constructor. Full
            # local decoding remains bounded to the short wake-listen window.
            rec = self.KaldiRecognizer(self.model, int(getattr(audio_source, "sample_rate", self.cfg.sample_rate)))
        latest_sequence = int(audio_source.latest_sequence())
        # Overlap adjacent short wake windows so a phrase beginning near the end
        # of one window is not discarded when the next recogniser is created.
        frame_ms = max(10, int(getattr(audio_source, "frame_ms", 20) or 20))
        overlap_frames = max(1, int(round(600.0 / frame_ms)))
        sequence = max(0, latest_sequence - overlap_frames)
        deadline = time.monotonic() + max(0.25, min(30.0, float(timeout_s or 2.0)))
        last_partial = ""
        sensitivity_value = max(0.40, min(0.95, float(sensitivity or 0.72)))
        required_partial_hits = 1 if sensitivity_value < 0.76 else (2 if sensitivity_value < 0.88 else 3)
        last_match = ""
        partial_hits = 0

        def detect(text: str) -> str:
            normal = " ".join(re.findall(r"[a-z0-9']+", str(text or "").lower()))
            if not normal:
                return ""
            padded = f" {normal} "
            for phrase in sorted(phrases, key=len, reverse=True):
                if f" {phrase} " in padded or normal.endswith(phrase):
                    return phrase
            return ""

        while time.monotonic() < deadline:
            if stop_event is not None and stop_event.is_set():
                return {"ok": True, "detected": False, "cancelled": True, "reason": "wake listen cancelled"}
            frames = audio_source.wait_for_frames(sequence, timeout_s=0.15)
            if not frames:
                continue
            for frame in frames:
                sequence = max(sequence, int(frame.get("seq", sequence)))
                pcm = bytes(frame.get("clean") or b"")
                if not pcm:
                    continue
                final_text = ""
                try:
                    if rec.AcceptWaveform(pcm):
                        final_text = str(json.loads(rec.Result() or "{}").get("text") or "")
                    partial = str(json.loads(rec.PartialResult() or "{}").get("partial") or "")
                except Exception:
                    partial = ""
                candidate_text = final_text or partial
                if candidate_text:
                    last_partial = candidate_text
                    match = detect(candidate_text)
                    if match:
                        if final_text:
                            partial_hits = required_partial_hits
                        elif match == last_match:
                            partial_hits += 1
                        else:
                            last_match = match
                            partial_hits = 1
                        if partial_hits >= required_partial_hits:
                            return {
                                "ok": True, "detected": True, "wake_phrase": match,
                                "recognised_text": candidate_text, "sequence": sequence,
                                "engine": "dynamic_vosk_grammar", "sensitivity": sensitivity_value,
                                "confirmation_hits": partial_hits,
                            }
                    elif partial:
                        last_match = ""
                        partial_hits = 0
        return {
            "ok": True, "detected": False, "reason": "wake window timeout",
            "recognised_text": last_partial, "sequence": sequence, "engine": "dynamic_vosk_grammar",
        }
    ...
    def _listen_once_alsa_detailed(
        ...
        cancel_event: Optional[threading.Event] = None, audio_source: Optional[Any] = None,
        force_start: bool = False, pre_roll_override_ms: Optional[int] = None,
    ) -> dict[str, Any]:
        if not shutil.which("arecord"):
        ...
        try:
            ...
            shared_capture = audio_source is not None and hasattr(audio_source, "capture_utterance")
            pre_roll_value = int(pre_roll_override_ms if pre_roll_override_ms is not None else (getattr(self.cfg, "stt_pre_roll_ms", 700) or 700))
            if shared_capture and endpointing:
                capture = audio_source.capture_utterance(
                ...
                capture = audio_source.capture_fixed(float(timeout_s), cancel_event=cancel_event)
            ...
            if shared_capture:
                ...
                rate = int(capture.get("sample_rate") or getattr(self.cfg, "sample_rate", 16000))
                if not clean_pcm:
                    return {"accepted": False, "text": "", "reason": "shared audio capture was empty", "error": "", "capture": capture, "capture_method": "shared_stream"}
                _write_pcm16_wav(raw_name, raw_pcm or clean_pcm, rate, 1)
                _write_pcm16_wav(clean_name, clean_pcm, rate, 1)
                _write_pcm16_wav(submitted_name, clean_pcm, rate, 1)
                processed = {
                    "ok": True, "source": "shared_stream_filtered", "sample_rate": rate, "channels": 1,
                    "duration_s": round(len(clean_pcm) / float(rate * 2), 3),
                    "analysis": analyse_wav_file(clean_name),
                }
            else:
            ...
            return {
                **decision,
                ...
                "capture_method": ("shared_stream_endpointed" if endpointing else "shared_stream_fixed") if shared_capture else ("alsa_endpointed" if endpointing else "alsa_fixed"),
    ...
    def listen_once_detailed(
        ...
        if method in {"auto", "alsa", "arecord"}:
            result = self._listen_once_alsa_detailed(
                ...
                audio_source=audio_source, force_start=force_start, pre_roll_override_ms=pre_roll_override_ms,
            )
