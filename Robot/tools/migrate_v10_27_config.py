#!/usr/bin/env python3
"""Migrate BX1 body configuration to v10.27 without losing calibration.
...
def migrate(config_path: Path, defaults_path: Path | None = None) -> Dict[str, Any]:
    ...
    if legacy_only:
        wake = list(NATURAL_WAKE_WORDS)
    current["wake_words"] = list(dict.fromkeys(wake))
    ...
    current["idle_life_enabled"] = bool(current.get("idle_life_enabled", True))
    current["idle_life_micro_actions_enabled"] = bool(current.get("idle_life_micro_actions_enabled", True))
    current["idle_life_self_chatter_enabled"] = True
    mode = str(current.get("idle_life_response_mode", "brain") or "brain").strip().lower()
    current["idle_life_response_mode"] = mode if mode in {"brain", "local", "off"} else "brain"
    ...
    if config_path.exists():
        ...
        backup = config_path.with_name(f"{config_path.name}.before_v10_27_{stamp}")
