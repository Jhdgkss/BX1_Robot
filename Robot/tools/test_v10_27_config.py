...
def main() -> int:
    ...
    required = [
        ...
        project / "tools/migrate_v10_27_config.py",
    ...
    check(str(cfg.get("version")) == VERSION and str(cfg.get("app_version")) == VERSION, "config version is 10.27", errors)
    check(cfg.get("input_mode") == "both" and cfg.get("voice_enabled") is True, "live keyboard + microphone enabled", errors)
    check([str(x).lower() for x in cfg.get("wake_words", [])] == ["hello", "hey", "robot"], "natural default wake words configured", errors)
    ...
    check(cfg.get("idle_life_require_alone") is True, "automatic idle speech is presence-gated", errors)
    ...
    compile_files = [str(p) for p in required[:4]] + [str(project / "tools/migrate_v10_27_config.py")]
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        migrate = project / "tools/migrate_v10_27_config.py"
        defaults = project / "python/config.v10.27.defaults.json"
        ...
        if migrated.returncode == 0:
            ...
            check(test_cfg.get("version") == VERSION, "migration writes v10.27", errors)
    ...
    if errors:
        ...
        return 1
    print("\nVALIDATION PASSED: BX1 v10.27 interaction and awareness update is internally consistent.")
    return 0
