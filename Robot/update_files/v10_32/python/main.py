...
def load_config() -> Dict[str, Any]:
    ...
    # v10.32: preserve desktop faster-whisper, but make wake detection more
    # tolerant and return robot-side thinking feedback ownership to the body.
    cfg["app_version"] = "10.32"
    cfg["version"] = "10.32"
    ...
    cfg.setdefault("thinking_feedback_enabled", True)
    cfg.setdefault("thinking_feedback_delay_s", 0.65)
    cfg.setdefault("thinking_cues_enabled", True)
    cfg.setdefault("thinking_cue_speak", True)
    cfg.setdefault("thinking_cue_delay_s", 2.1)
    cfg.setdefault("thinking_cue_repeat_s", 5.0)
...
class BX1RobotBodyService:
    ...
    def _apply_primary_stt(self, result: Dict[str, Any], source: str = "live") -> Dict[str, Any]:
        ...
        if not self.brain.base_url:
            result["primary_stt_error"] = "Brain App URL is not configured; local Vosk fallback used."
            return result
        ...
        try:
            ...
            configured_prompt = str(self.cfg.get("brain_stt_initial_prompt", "") or "").strip()
            wake_prompt = "The speaker may begin with a wake phrase such as " + ", ".join(self.get_wake_words()) + ". Preserve the wake phrase in the transcript."
            initial_prompt = (configured_prompt + " " + wake_prompt).strip()
        ...
        if bool(remote.get("ok")) and remote_text:
            ...
            accepted = bool(tokens) and not (tokens and all(t.lower() in fillers for t in tokens))
            result.update({
                "accepted": accepted,
                "reason": "accepted" if accepted else "filler/noise fragment",
                "text": remote_text,
        ...
        if fallback and local_text:
            result["transcription_backend"] = "local_vosk_fallback"
            return result
        result.update({
            "accepted": False,
    ...
    def _wake_aliases_for(self, configured: str) -> List[str]:
        ...
        built_in = {
            ...
            "hey": ["hay"],
            "robot": ["robo", "row bot", "rowboat", "robert"],
            "leo": ["leon", "leah"],
            "bx1": ["b x one", "bee ex one", "box one"],
        }
    ...
    def _match_wake_word_detailed(self, text: str, wake_words: List[str]) -> tuple[str, float, str]:
        ...
        for wake in sorted(normalised, key=len, reverse=True):
            ...
            for variant in sorted((v for v in variants if v), key=len, reverse=True):
                if re.search(r"(?:^|\s)" + re.escape(variant) + r"(?:$|\s)", low):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                try:
                    ...
                    stt_result = self._apply_primary_stt(stt_result, source="live_voice")
    ...
    def handle_user_text(self, text: str, use_web: Optional[bool] = None, use_memory: Optional[bool] = None,
        ...
        def request_watchdog() -> None:
            ...
            self.web_log("warning", f"Brain chat request exceeded {delay:.0f}s", {"event_id": event_id, "message": text[:160]})
        ...
        if live_tool:
            ...
            with self.metrics_lock:
                self.performance.update({
                    "last_live_tool_route": route,
                    "last_live_tool_verified": verified,
                    "last_live_tool_query": query,
                    "last_live_tool_sources": sources,
                    "last_live_tool_result_count": result_count,
                })
            status = "verified" if verified else "failed"
            source_text = ", ".join(sources) if sources else "no named source"
            self.web_log(
                "system" if verified else "warning",
                f"Live tool {route}: {status}; {result_count} result(s); {source_text}",
                live_tool,
            )
    ...
    def start_thinking_cues(self, reason: str = "chat") -> threading.Event:
        ...
        def worker() -> None:
            try:
                chirp_delay = max(0.10, float(self.cfg.get("thinking_feedback_delay_s", 0.35)))
                spoken_delay = max(chirp_delay + 0.25, float(self.cfg.get("thinking_cue_delay_s", 1.15)))
                spoken_repeat = max(2.5, float(self.cfg.get("thinking_cue_repeat_s", 5.5)))
                max_spoken = max(0, min(3, int(self.cfg.get("thinking_cue_max_per_reply", 2)))) if spoken_enabled else 0
    ...
    def web_stt_once(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self._apply_primary_stt(result, source="web_stt_diagnostic")
        finally:
            if got_lock:
                try:
                    self.audio_capture_lock.release()
