...
def play_audio_file(filename: str | os.PathLike, device: str = "default") -> dict:
    path = Path(filename)
    if not path.exists():
        return {"ok": False, "error": f"file not found: {path}"}
    player = shutil.which("aplay") or shutil.which("paplay") or shutil.which("ffplay")
    ...
    if player_name.startswith("ffplay"):
        ...
        cmd = [player, str(path)]
    try:
        res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=30, check=False)
        return {
            "ok": res.returncode == 0,
            "command": cmd,
...
class VoskSpeechToText:
    ...
    def _validate(self, result: dict[str, Any], voice: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"accepted": accepted, "reason": reason, "text": text, "tokens": tokens}
