...
<title>BX1 Robot Control 10.30</title>
<style>
:root{
...
<div class="app">
<aside class="sidebar">
  <div class="logo"><div class="eyebrow">BX1 / LEO</div><h1>ROBOT<br>CONTROL</h1><small>Robot client v10.30</small></div>
...
<section class="page" id="page-speech">
  ...
  <div class="grid">
    ...
    <article class="card span-5"><h4>Production STT Test</h4><div class="help">Waits for speech, keeps the first syllable, and closes after natural end silence. It does not use a fixed five-second chop.</div><div class="buttons"><button class="good" onclick="listenOnce(false,this)">Listen once</button><button class="primary" onclick="listenOnce(true,this)">Listen and send to Brain</button></div><div id="sttResult" class="result-box">No diagnostic utterance captured yet.</div><div class="buttons"><a href="/api/stt_audio/raw.wav" target="_blank"><button>Play raw</button></a><a href="/api/stt_audio/filtered.wav" target="_blank"><button>Play filtered</button></a><a href="/api/stt_audio/submitted.wav" target="_blank"><button>Play submitted</button></a></div></article>
...
<section class="page" id="page-hardware">
  ...
  <div class="grid">
    <article class="card span-5"><h4>Live Hardware State</h4><div id="hardwareMetrics" class="metric-grid"></div><div class="buttons"><button onclick="hardwareTest('centre',0,null,this)">Centre head</button><button onclick="hardwareTest('yaw',-10,null,this)">Yaw left</button><button onclick="hardwareTest('yaw',10,null,this)">Yaw right</button><button onclick="hardwareTest('pitch',7,null,this)">Pitch up</button><button onclick="hardwareTest('pitch',-7,null,this)">Pitch down</button></div></article>
...
<script>
let SNAP={};let EVENTS=[];let levels=[];let refreshing=false;let page='overview';let visionPreviewTimer=null;
...
function deriveHealth(d){const s=d.state||{},vr=d.voice_runtime||{},ml=((d.mic_level||{}).level)||{},mouth=d.mouth_audio||{};const brainBad=!!s.brain_error;const sttReady=(d.voice||{}).stt_ready;const liveMic=!!vr.loop_active&&!['disabled','error'].includes(String(vr.state||''));const micGood=ml.running||liveMic;const micState=ml.error?'bad':micGood?'good':'warn';const mcu=!!s.mcu_ok;const imu=!!((s.sensors||{}).imu_ok);const mouthState=mouth.last_ok===false?'bad':mouth.commands>0?'good':'warn';const micLabel=liveMic?(vr.state==='recording'?'VOICE':'LISTENING'):(ml.running?(ml.active?'VOICE':'MONITORING'):'STOPPED');return [statusCard('Brain',brainBad?'FAULT':'CONNECTED',brainBad?'bad':'good'),statusCard('STT',sttReady?'READY':((d.voice||{}).stt_error?'FAULT':'IDLE'),sttReady?'good':(d.voice||{}).stt_error?'bad':'warn'),statusCard('Microphone',micLabel,micState),statusCard('MCU Bridge',mcu?'ONLINE':'OFFLINE',mcu?'good':'bad'),statusCard('IMU',imu?'ONLINE':(mcu?'DEGRADED':'UNKNOWN'),imu?'good':mcu?'warn':'off'),statusCard('Mouth LED',mouth.active?'ANIMATING':mouth.commands>0?'READY':'NO EVIDENCE',mouthState)].join('')}
function buildFaults(d){const s=d.state||{},v=d.voice||{},ml=((d.mic_level||{}).level)||{},doc=d.hardware_doctor||{},m=d.mouth_audio||{};let f=[];if(s.brain_error)f.push(['Brain','fault',s.brain_error,'Open Advanced and test Brain connection']);if(v.stt_error)f.push(['Speech','fault',v.stt_error,'Open Speech Input']);if(ml.error)f.push(['Microphone','fault',ml.error,'Check selected ALSA device']);if(!s.mcu_ok)f.push(['MCU','fault',s.bridge_error||s.mcu_error||doc.summary||'MCU bridge not healthy','Run Hardware Doctor']);if(s.mcu_ok&&!((s.sensors||{}).imu_ok))f.push(['IMU','warning',(s.sensors||{}).imu_error||'IMU is not reporting valid data','Check Qwiic connection/address']);if(m.last_ok===false)f.push(['Mouth LED','fault',m.last_error||'Last mouth command failed','Open Mouth / Lighting']);const rejected=(d.events||[]).slice().reverse().find(e=>e.kind==='rejected_input');if(rejected)f.push(['STT recent','warning',eventText(rejected),'Review endpoint result and debug WAVs']);if(!f.length)f.push(['System','ok','No active body faults detected.','']);return f}
...
function renderConversation(d){const el=$('chatTranscript');if(!el)return;const vr=d.voice_runtime||{},p=d.performance||{},v=d.voice||{};const messages=(d.events||[]).filter(e=>['user','input','bx1','error'].includes(String(e.kind||''))&&eventText(e)).slice(-80);el.innerHTML=messages.length?messages.map(e=>{const k=String(e.kind||'');const c=k==='bx1'?'robot':(k==='error'?'error':'user');const who=k==='bx1'?'BX1':(k==='error'?'System':'You');return `<div class="chat-msg ${c}"><div class="who">${who} · ${esc(e.at||e.timestamp||'')}</div>${esc(eventText(e))}</div>`}).join(''):'<div class="muted">No conversation yet. Type a message or use Listen and send.</div>';$('conversationMetrics').innerHTML=metric('Voice',vr.state||'--')+metric('Listener',vr.loop_active?'active':'stopped')+metric('STT',v.stt_ready?'ready':(v.stt_error?'fault':'starting'))+metric('Brain',p.last_brain_latency_ms!=null?p.last_brain_latency_ms+' ms':'--');$('conversationStatus').className='result-box '+(vr.state==='error'?'bad':vr.loop_active?'good':'');$('conversationStatus').textContent=vr.label||vr.message||(vr.loop_active?'Listening for Hello, Hey or Robot.':'Live listening is not active. Open Speech Input and activate it.');const metrics=vr.last_stt_metrics||{};const heard=String(vr.last_heard||'').trim();const accepted=String(vr.last_accepted||'').trim();const cue=String(vr.last_thinking_cue||'').trim();let heardLines=[];if(heard)heardLines.push('Heard: “'+heard+'”');if(accepted&&accepted!==heard)heardLines.push('Command: “'+accepted+'”');if(vr.last_wake_word)heardLines.push('Wake word: '+vr.last_wake_word);if(metrics.confidence!==undefined&&metrics.confidence!==null&&metrics.confidence!=='')heardLines.push('Confidence: '+Number(metrics.confidence).toFixed(2));if(vr.state==='processing')heardLines.push('Thinking phase: '+String(vr.thinking_phase||'processing')+(cue?' — '+cue:''));const h=$('conversationHeard');h.className='result-box '+(vr.state==='error'?'bad':heard?'good':'');h.textContent=heardLines.length?heardLines.join('\n'):'No speech recognised yet.';if(page==='conversation'&&!isDirty($('chatText')))el.scrollTop=el.scrollHeight}
function renderSpeech(d){const m=d.mic||{},v=d.voice||{},vr=d.voice_runtime||{},a=d.audio||{};fillSelect($('micDevice'),m.devices,m.mic_device,x=>({value:x.device||x.id||x.name,name:(x.device||x.id||x.name)+' — '+(x.description||x.label||'ALSA input')}));$('sampleRate').value=String(m.sample_rate||16000);$('micGain').value=m.mic_software_gain_db??0;$('noiseGate').value=m.mic_noise_gate_dbfs??-48;$('adaptiveMargin').value=m.stt_adaptive_margin_db??8;$('preRoll').value=m.stt_pre_roll_ms??700;$('endSilence').value=m.stt_end_silence_ms??1350;$('postRoll').value=m.stt_post_roll_ms??300;$('startTimeout').value=m.stt_start_timeout_s??8;$('maxUtterance').value=m.stt_max_utterance_s??20;$('startTrigger').value=m.stt_start_trigger_ms??80;$('endpointingEnabled').checked=m.stt_endpointing_enabled!==false;$('adaptiveEnabled').checked=m.stt_adaptive_threshold_enabled!==false;$('highpassHz').value=m.audio_highpass_hz??90;$('notchHz').value=m.audio_notch_hz??50;$('noiseReduction').value=m.audio_noise_reduction_strength??.2;$('filterEnabled').checked=m.audio_filter_enabled!==false;$('highpassEnabled').checked=m.audio_highpass_enabled!==false;$('notchEnabled').checked=m.audio_notch_enabled!==false;$('nrEnabled').checked=m.audio_noise_reduction_enabled!==false;$('minConfidence').value=m.stt_min_confidence??.4;$('minVoiced').value=m.stt_min_voiced_ms??280;$('wakeWords').value=(v.wake_words||[]).join('\n');$('voiceEnabled').checked=!!v.voice_enabled;$('voiceMetrics').innerHTML=metric('State',vr.state||'--')+metric('STT ready',yes(v.stt_ready),v.stt_error||'')+metric('Capture',v.capture_method||'alsa')+metric('Endpointing',v.endpointing_enabled?'natural':'fixed');$('voskModelPath').value=v.vosk_model_path||'';const improved=v.vosk_model_tier==='improved'||String(v.vosk_model_path||'').includes('0.22-lgraph');$('speechModelHint').className='result-box '+(improved?'good':'');$('speechModelHint').textContent=improved?'Improved Vosk lgraph model active. The longer pre-roll also protects the first words of a sentence.':'Small Vosk model active. For more accurate full-sentence recognition, run INSTALL_BX1_BETTER_STT_MODEL.sh in the project folder, then retest.';$('ttsVolume').value=a.tts_volume??80;$('voiceOwnerText').textContent='TTS engine and voice owner: '+(a.voice_owner||'desktop_brain_app')+'. This page controls only speaker output.';renderLevel(((d.mic_level||{}).level)||{})}
...
async function refreshAll(manual=false){if(refreshing)return;if(!manual&&document.querySelector('[data-dirty="1"]'))return;refreshing=true;try{const d=await req('/api/status');renderAll(d);if(manual)toast('Status refreshed','good')}catch(e){toast('Status failed: '+e.message,'bad')}finally{refreshing=false}}
...
async function loadDevices(btn){busy(btn,true);try{const d=await req('/api/mic_devices');const m=SNAP.mic||{};fillSelect($('micDevice'),d.devices,m.mic_device,x=>({value:x.device||x.id||x.name,name:(x.device||x.id||x.name)+' — '+(x.description||x.label||'ALSA input')}));const p=await req('/api/mic_playback_devices');fillSelect($('playbackDevice'),p.devices,(SNAP.audio||{}).tts_playback_device,x=>({value:x.device||x.id||x.name,name:(x.device||x.id||x.name)+' — '+(x.description||x.label||'ALSA output')}));toast('Audio devices refreshed','good')}catch(e){toast(e.message,'bad')}finally{busy(btn,false)}}
...
async function saveMic(btn){busy(btn,true,'Saving…');try{await req('/api/mic_settings',micPayload());toast('Speech input settings saved','good');await refreshAll()}catch(e){toast(e.message,'bad')}finally{busy(btn,false)}}
...
async function listenOnce(send,btn){busy(btn,true,'Listening…');$('sttResult').className='result-box';$('sttResult').textContent='Waiting for speech…';try{const d=await req('/api/stt_once',{send});const r=d.stt||{},c=r.capture||{},va=r.voice_activity||{};$('sttResult').className='result-box '+(d.ok?'good':'bad');$('sttResult').innerHTML=`<div class="result-big">${esc(d.text||'[no accepted text]')}</div>Accepted: ${yes(r.accepted)} · reason: ${esc(r.reason||'accepted')}<br>Close: ${esc(c.close_reason||'--')} · duration: ${esc(c.duration_s??'--')} s · waited: ${esc(c.waited_for_speech_s??'--')} s<br>Threshold: ${esc(c.threshold_dbfs??va.threshold_dbfs??'--')} dBFS · noise floor: ${esc(c.noise_floor_dbfs??va.noise_floor_dbfs??'--')} dBFS · confidence: ${esc(r.confidence??'--')}`;toast(d.ok?'Speech recognised':'Speech rejected: '+(d.error||r.reason),' '+(d.ok?'good':'bad'));await refreshAll()}catch(e){$('sttResult').className='result-box bad';$('sttResult').textContent='STT test failed: '+e.message;toast(e.message,'bad')}finally{busy(btn,false)}}
...
async function saveBrain(useIp,btn){busy(btn,true);try{const d=await req('/api/brain_settings',{base_url:$('brainUrl').value,use_browser_client_ip:useIp,sync_tts_to_brain_host:true});$('brainResult').textContent=JSON.stringify(d,null,2);toast('Brain connection saved','good');await refreshAll()}catch(e){toast(e.message,'bad')}finally{busy(btn,false)}}
...
$('chatText').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendChat(document.querySelector('#page-conversation button.primary'))}});initRoute();tickClock();setInterval(tickClock,1000);refreshAll();setInterval(()=>refreshAll(false),5000);setInterval(pollLevel,450);loadDevices(null);
</script>
</body>
</html>
...
class WebControlServer:
    ...
    def start(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "RobotBodyClient/10.30"
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                if path in {"/api/stt_audio/raw.wav", "/api/stt_audio/filtered.wav", "/api/stt_audio/submitted.wav"}:
                    names = {
                        "/api/stt_audio/raw.wav": "bx1_stt_last_raw.wav",
                        "/api/stt_audio/filtered.wav": "bx1_stt_last_filtered.wav",
                        "/api/stt_audio/submitted.wav": "bx1_stt_last_submitted.wav",
                    }
                    sample = Path("/tmp") / names[path]
