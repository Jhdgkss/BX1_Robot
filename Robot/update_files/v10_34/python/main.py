...
def load_config() -> Dict[str, Any]:
    ...
    cfg.setdefault("thinking_cue_repeat_s", 8.0)
    cfg.setdefault("thinking_cue_max_per_reply", 5)
    cfg.setdefault("thinking_cue_total_timeout_s", 65.0)
    cfg.setdefault("thinking_cue_finish_wait_s", 2.2)
    ...
    # Chatterbox is generating the final reply.
...
class BX1RobotBodyService:
    ...
    def handle_mouth_audio_event(self, event: str, info: Dict[str, Any]) -> None:
        ...
        # a USB microphone near the speaker can hear BX1's own Chatterbox reply.
        ...
        if event == "speech_prepare" and not is_local_cue:
            # A long answer may be split into several Chatterbox chunks. The
            ...
            self.set_voice_runtime(
                "speechgen",
                "Building the Chatterbox voice; progress cues remain active.",
        ...
        if event in {"speech_start", "speech_audio_file_start"}:
            if is_local_cue:
                ...
                self.set_voice_runtime(
                    phase_state,
                    text or ("Building the Chatterbox voice…" if phase_state == "speechgen" else "Thinking…"),
            ...
            if is_local_cue:
                ...
                # final reply. The Chatterbox request is still being generated.
                current = self.get_voice_runtime_snapshot()
                state = "speechgen" if str(current.get("state")) == "speechgen" else "processing"
                label = "Building the Chatterbox voice; progress cues remain active." if state == "speechgen" else "Thinking…"
                self.set_voice_runtime(
                    state,
                    label,
                    loop_active=bool(current.get("loop_active", False)),
    ...
    def _expression_head_speech_worker(self, plan: Dict[str, Any]) -> None:
        ...
        if bool(self.cfg.get("expression_led_idle_after_reply", True)):
            # Do not display IDLE while Chatterbox is still generating or queued.
    ...
    def handle_user_text(self, text: str, use_web: Optional[bool] = None, use_memory: Optional[bool] = None,
        ...
        try:
            ...
            # Chatterbox generation is usually the longest part of the pipeline.
    ...
    def handle_vision(self, prompt: str, use_web: Optional[bool] = None,
        ...
        try:
            ...
            # Keep progress cues alive through Chatterbox generation. They stop
    ...
    def handle_brain_result(self, result: Dict[str, Any], original_message: str,
        ...
        if reply:
            ...
            if audio_already_played_on_robot:
                ...
                self.set_voice_runtime(
                    "processing",
                    "Reply ready; generating Chatterbox voice...",
                    loop_active=bool(self.get_voice_runtime_snapshot().get("loop_active", False)),
                    thinking_phase="tts_generation",
                )
                if thinking_stop is not None:
                    self.register_active_thinking_cues(thinking_stop)
                self.tts.speak(reply, flush=True, tag="reply")
                if not bool(self.cfg.get("tts_enabled", True)):
                    if thinking_stop is not None:
                        thinking_stop.set()
                    self.stop_active_thinking_cues("tts_disabled")
        else:
            if thinking_stop is not None:
                thinking_stop.set()
    ...
    def start_thinking_cues(self, reason: str = "chat") -> threading.Event:
        ...
        def speak_cue(index: int) -> None:
            ...
            # BX1 has not stalled while Chatterbox renders the final voice.
    ...
    def web_update_thinking_cue_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # v10.34 makes local acknowledgement/progress timing a body responsibility.
