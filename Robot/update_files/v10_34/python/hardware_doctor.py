...
class BX1HardwareDoctor:
    ...
    def _imu_probe_template(note: str) -> str:
        return f'''/* BX1 generated IMU probe. Review only; no motor outputs.\nNote: {note}\n*/\n#include <Arduino.h>\n#include <Arduino_RouterBridge.h>\n#include <Arduino_LSM6DSOX.h>\nbool imuOk=false; float ax=0, ay=0, az=0;\nString bx1_get_status() {{\n  if (imuOk && IMU.accelerationAvailable()) IMU.readAcceleration(ax,ay,az);\n  return String("{{\\\"mcu_ok\\\":true,\\\"probe\\\":\\\"imu\\\",\\\"imu_ok\\\":") + (imuOk?"true":"false") + ",\\\"ax\\\":" + String(ax,4) + ",\\\"ay\\\":" + String(ay,4) + ",\\\"az\\\":" + String(az,4) + ",\\\"maintenance_mode\\\":true}}";\n}}\nvoid setup() {{ Serial.begin(115200); imuOk=IMU.begin(); Bridge.begin(); Bridge.provide("bx1_get_status", bx1_get_status); }}\nvoid loop() {{ delay(50); }}\n'''
