...
class BX1RobotBodyService:
    ...
    def start_thinking_cues(self, reason: str = "chat") -> threading.Event:
        ...
        if bool(self.cfg.get("brain_controls_thinking_cues", True)):
            # The desktop Brain App owns thinking speech/content. The body only
            # shows the processing LED state and must not speak a second cue.
            return stop
        if not bool(self.cfg.get("thinking_cues_enabled", False)):
            return stop
