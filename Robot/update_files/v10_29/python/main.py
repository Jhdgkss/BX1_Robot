...
# High-level personality controls were removed from the body client in v10.24.1.
...
class BX1RobotBodyService:
    ...
    def _send_mouth_colour(self, colour: str, brightness: float) -> None:
        ...
        try:
            ...
            if not result.ok:
                self.web_log("error", f"mouth LED command failed: {result.error}", {"action": action, "value": result.value})
        except Exception as exc:
            self.mouth_audio_runtime.update({"last_ok": False, "last_error": str(exc), "last_command_at": now_iso()})
    ...
    def handle_mouth_audio_event(self, event: str, info: Dict[str, Any]) -> None:
        ...
        if event in {"speech_start", "speech_audio_file_start"}:
            ...
            self.set_voice_runtime(
                ...
                pre_speech_state=str(before.get("state", "idle")),
            )
    ...
    def _match_wake_word(self, text: str, wake_words: List[str]) -> str:
        ...
        # Small Vosk models commonly turn a wake word into a close homophone.
        # Limit aliases to the first two words so ordinary conversation does not
        # wake the robot accidentally. The user-visible wake words remain editable.
        first_words = low.split()[:2]
        aliases = {
            "hello": {"hi", "hullo", "yellow"},
            "hey": {"hay"},
            "robot": {"robo", "rowboat", "robert"},
        }
        for configured in normalised:
            for alias in aliases.get(configured, set()):
                if alias in first_words:
                    return configured
        return ""
    ...
    def _strip_wake_words(self, text: str, wake_words: List[str]) -> str:
        ...
        for wake in sorted((self._normalise_spoken_text(w) for w in wake_words if str(w).strip()), key=len, reverse=True):
            variants = {wake, *aliases.get(wake, set())}
    ...
    def handle_brain_result(self, result: Dict[str, Any], original_message: str) -> Dict[str, Any]:
        ...
        if reply:
            ...
            if audio_already_played_on_robot:
                ...
                # Some Brain versions play the audio remotely and only return a
                # completion flag.  In that case no local TTS callback reaches the
                # body, so provide a bounded visual speech animation instead of
                # leaving the mouth and state LEDs static.
    ...
    def start_thinking_cues(self, reason: str = "chat") -> threading.Event:
        """Render immediate progress feedback while the Brain request is pending.

        The desktop Brain still owns personality and spoken filler wording.  The
        body is allowed to render a short non-verbal tone and thinking LEDs,
        because those must happen before the synchronous Brain reply returns.
        """
        stop = threading.Event()
        brain_owns_words = bool(self.cfg.get("brain_controls_thinking_cues", True))
        nonverbal_enabled = bool(self.cfg.get("thinking_feedback_enabled", True))
        spoken_enabled = (
            not brain_owns_words
            and bool(self.cfg.get("thinking_cues_enabled", False))
            and bool(self.cfg.get("thinking_cue_speak", True))
        )
        ...
        def worker() -> None:
            try:
                delay = max(0.15, float(self.cfg.get("thinking_feedback_delay_s", 0.45)))
                repeat = max(1.5, float(self.cfg.get("thinking_feedback_repeat_s", 4.0)))
                max_feedback = max(0, min(5, int(self.cfg.get("thinking_feedback_max_per_reply", 2))))
                max_spoken = max(0, int(self.cfg.get("thinking_cue_max_per_reply", 1))) if spoken_enabled else 0
                cycles = max(max_feedback if nonverbal_enabled else 0, max_spoken)
                if cycles <= 0 or stop.wait(delay):
                    return
                ...
                for idx in range(cycles):
                    ...
                    if nonverbal_enabled and idx < max_feedback:
                        self.web_log("thinking", "thinking progress tone", {"reason": reason, "index": idx + 1})
                        self.play_voice_feedback("thinking", blocking=True)
                    if spoken_enabled and idx < max_spoken and not stop.is_set():
                        cue = random.choice(cues) if cues else "One moment."
                        cue_key = ""
                        ...
                        self.play_local_voice_cue(cue_key, cue, allow_main_tts_fallback=False)
                    if idx >= cycles - 1 or stop.wait(repeat):
                        return
            except Exception as exc:
    ...
    def play_one_thinking_cue(self) -> Dict[str, Any]:
        if bool(self.cfg.get("brain_controls_thinking_cues", True)):
            return {
                "ok": False,
                "skipped": "spoken thinking cues are owned by the desktop Brain App",
    ...
    def apply_audio_settings(self, data: Dict[str, Any], save: bool = True) -> Dict[str, Any]:
        ...
        # The desktop Brain App is the sole owner of the active TTS engine and
        # voice. Older body web pages may still call this compatibility endpoint,
        # but they cannot switch Leo to a second local voice configuration. The
        # body continues to own only playback enable/volume/device and transport.
        backend = "brain-tts"
        ...
        if save:
            self.save_robot_profile_file()
            self.save_config_file()
        self.web_log("system", f"Body playback settings saved: voice_owner=desktop_brain_app, backend={backend}, volume={self.cfg['tts_volume']}%")
    ...
    def web_stt_once(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {}
        try:
            got_lock = self.audio_capture_lock.acquire(timeout=max(3.0, float(self.cfg.get("stt_start_timeout_s", 8.0)) + 2.0))
            if not got_lock:
                return {"ok": False, "stage": "capture", "error": "microphone is busy"}
            if monitor_was_running:
                self.mic_monitor.stop()
                time.sleep(0.15)
            ...
            result = stt.listen_once_detailed(float(self.cfg.get("stt_start_timeout_s", self.cfg.get("record_seconds", 8))))
        finally:
            if got_lock:
                try:
                    self.audio_capture_lock.release()
    ...
    def get_thinking_cue_settings(self) -> Dict[str, Any]:
        brain_owns = bool(self.cfg.get("brain_controls_thinking_cues", True))
        return {
            "owner": "desktop_brain_app" if brain_owns else "body_compatibility_mode",
            "thinking_cues_enabled": False if brain_owns else bool(self.cfg.get("thinking_cues_enabled", False)),
            "thinking_cue_speak": False if brain_owns else bool(self.cfg.get("thinking_cue_speak", False)),
    ...
    def web_update_thinking_cue_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        brain_owns = bool(self.cfg.get("brain_controls_thinking_cues", True))
        if brain_owns:
            self.cfg["thinking_cues_enabled"] = False
            self.cfg["thinking_cue_speak"] = False
            self.cfg["voice_command_immediate_cue_enabled"] = False
        else:
            self.cfg["thinking_cues_enabled"] = bool(data.get("thinking_cues_enabled", self.cfg.get("thinking_cues_enabled", False)))
            self.cfg["thinking_cue_speak"] = bool(data.get("thinking_cue_speak", self.cfg.get("thinking_cue_speak", False)))
            ...
            self.cfg["voice_command_immediate_cue_enabled"] = bool(data.get("voice_command_immediate_cue_enabled", self.cfg.get("voice_command_immediate_cue_enabled", False)))
