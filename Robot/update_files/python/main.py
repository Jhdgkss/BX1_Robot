...
DEFAULT_HARDWARE_REGISTRY = {
    ...
    "servos": {
        ...
        "gimbal_left": {"label": "Left push-pull gimbal servo", "enabled": False, "pin": 6, "min_deg": -20, "home_deg": 0, "max_deg": 20, "invert": False, "speed_deg_s": 75},
        "gimbal_right": {"label": "Right push-pull gimbal servo", "enabled": False, "pin": 9, "min_deg": -20, "home_deg": 0, "max_deg": 20, "invert": False, "speed_deg_s": 75},
    },
    "head_kinematics": {
    ...
    "sensors": {
        "modulino_movement": {"label": "Arduino Modulino Movement IMU", "enabled": True, "bus": "i2c_qwiic", "i2c_address": "0x6A", "use_for": ["pitch", "roll", "gyro", "motion_awareness"]}
    },
    "drive_buses": {
        "rs485_wheels": {"label": "Future RS485 closed-loop wheel steppers", "enabled": False, "adapter": "usb_rs485_or_uart_rs485", "port": "", "baud": 115200, "left_motor_id": 1, "right_motor_id": 2, "max_linear_mps": 0.25, "max_angular_dps": 60, "safety_timeout_ms": 500}
    },
}

# Legacy shape retained for older parts of the service and older sketches.
DEFAULT_HARDWARE_MAP = {
    "head_yaw": {"label": "Head rotation", "role": "head_yaw", "device_type": "servo", "enabled": False, "pin": 5, "min_deg": -20, "max_deg": 20, "home_deg": 0, "invert": False},
    "head_pitch": {"label": "Left gimbal servo (legacy pitch slot)", "role": "head_pitch", "device_type": "servo", "enabled": False, "pin": 6, "min_deg": -20, "max_deg": 20, "home_deg": 0, "invert": False},
    "head_roll": {"label": "Right gimbal servo (legacy roll slot)", "role": "head_roll", "device_type": "servo", "enabled": False, "pin": 9, "min_deg": -20, "max_deg": 20, "home_deg": 0, "invert": False},
...
def normalise_hardware_registry(raw: Any) -> Dict[str, Any]:
    ...
    if isinstance(sensors.get("modulino_movement"), dict):
        ...
        result["sensors"]["modulino_movement"].update({"enabled": _bool_or(inc.get("enabled", True)), "bus": str(inc.get("bus", "i2c_qwiic")), "i2c_address": str(inc.get("i2c_address", "0x6A"))})
...
PERSONALITY_CONTROL_KEYS = [
    ("humour", "Humour", 35),
    ("honesty", "Honesty", 80),
    ("sarcasm", "Dry humour / sarcasm", 20),
    ("timidity", "Timidity / caution in voice", 45),
    ("curiosity", "Curiosity", 75),
    ("chattiness", "Chattiness", 45),
    ("technical", "Technical depth", 75),
    ("obedience", "Obedience", 55),
    ("confidence", "Confidence", 65),
    ("energy", "Energy", 55),
    ("empathy", "Empathy", 55),
    ("caution", "Safety caution", 70),
]

DEFAULT_PERSONALITY_CONTROLS = {key: default for key, _label, default in PERSONALITY_CONTROL_KEYS}

PERSONALITY_CONTROL_ALIASES = {
    "humor": "humour",
    "humour": "humour",
    "jokes": "humour",
    "joke": "humour",
...
PERSONALITY_PRESETS = {
    "tars": {
        "humour": 80, "honesty": 90, "sarcasm": 55, "timidity": 20,
        "curiosity": 75, "chattiness": 45, "technical": 80, "obedience": 35,
        "confidence": 85, "energy": 55, "empathy": 45, "caution": 65,
    },
    "engineering": {
        "humour": 25, "honesty": 90, "sarcasm": 10, "timidity": 20,
        "curiosity": 70, "chattiness": 30, "technical": 95, "obedience": 55,
        "confidence": 85, "energy": 45, "empathy": 40, "caution": 80,
    },
    "friendly": {
        "humour": 55, "honesty": 80, "sarcasm": 10, "timidity": 35,
        "curiosity": 80, "chattiness": 60, "technical": 65, "obedience": 60,
        "confidence": 70, "energy": 65, "empathy": 75, "caution": 70,
    },
    "quiet": {
        "humour": 15, "honesty": 85, "sarcasm": 5, "timidity": 45,
        "curiosity": 45, "chattiness": 15, "technical": 65, "obedience": 65,
        "confidence": 60, "energy": 25, "empathy": 55, "caution": 75,
    },
    "debug": {
        "humour": 5, "honesty": 95, "sarcasm": 0, "timidity": 10,
        "curiosity": 40, "chattiness": 20, "technical": 100, "obedience": 70,
        "confidence": 85, "energy": 40, "empathy": 30, "caution": 90,
    },
}


def clamp_percent_value(value: Any, default: int = 50) -> int:
    try:
        n = float(value)
        if 0.0 <= n <= 1.0:
            n *= 100.0
...
def normalise_personality_controls(raw: Any, personality: Optional[Dict[str, Any]] = None) -> Dict[str, int]:
    controls = dict(DEFAULT_PERSONALITY_CONTROLS)
    incoming = raw if isinstance(raw, dict) else {}
    for key, value in incoming.items():
        canonical = PERSONALITY_CONTROL_ALIASES.get(str(key).strip().lower(), str(key).strip().lower())
        if canonical in controls:
            controls[canonical] = clamp_percent_value(value, controls[canonical])
    personality = personality if isinstance(personality, dict) else {}
    if "humour" not in incoming and "humor" not in incoming and "humour_level" in personality:
        controls["humour"] = clamp_percent_value(personality.get("humour_level"), controls["humour"])
    if "confidence" not in incoming and "confidence_level" in personality:
        controls["confidence"] = clamp_percent_value(personality.get("confidence_level"), controls["confidence"])
    return controls


def personality_controls_line(controls: Dict[str, int]) -> str:
    return ", ".join(f"{label} {int(controls.get(key, default))}%" for key, label, default in PERSONALITY_CONTROL_KEYS)
...
def default_robot_profile(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    wake_words = normalise_text_list(config.get("wake_words", ["robot"]))
    return {
        "schema_version": 2,
        "body_id": body_id,
        "robot_id": body_id,
...
class BX1RobotBodyService:
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.brain = BX1BrainClient(BrainClientConfig(
            base_url=str(config.get("brain_base_url", "http://127.0.0.1:8765")),
    ...
    def play_voice_feedback(self, kind: str, blocking: bool = True) -> Dict[str, Any]:
        """Give an immediate, deterministic audio cue from the robot speaker."""
        ...
        def worker() -> Dict[str, Any]:
            try:
                ...
                result = play_audio_file(path, device=str(self.cfg.get("tts_playback_device", "default") or "default"))
                if not result.get("ok"):
                    self.web_log("error", f"voice feedback tone failed: {result.get('error') or result.get('stderr') or result}")
                return result
    ...
    def get_idle_life_snapshot(self) -> Dict[str, Any]:
        ...
        snap.update({
            ...
            "micro_action_delay_s": float(self.cfg.get("idle_life_micro_action_delay_s", 120.0)),
            "comment_delay_s": float(self.cfg.get("idle_life_comment_delay_s", 300.0)),
    ...
    def build_audio_config(self) -> AudioConfig:
        # Startup must not depend on the Windows Brain App already being configured.
        # If Brain TTS is selected but no Brain URL is known yet, boot with local
        # espeak fallback; the web Brain Connection page will switch back to
        # brain-tts once the Brain PC URL is saved.
        brain_tts_url = self.resolve_brain_tts_base_url(update_config=False)
        selected_tts_backend = str(self.cfg.get("tts_backend", "espeak-ng"))
        ...
        return AudioConfig(
            ...
            brain_tts_engine=str(self.cfg.get("brain_tts_engine", "chatterbox_turbo")),
            brain_tts_voice=str(self.cfg.get("brain_tts_voice", "chatterbox_bx1")),
            brain_tts_format=str(self.cfg.get("brain_tts_format", "wav")),
            brain_tts_timeout_s=int(self.cfg.get("brain_tts_timeout_s", 240)),
            brain_tts_endpoint=str(self.cfg.get("brain_tts_endpoint", "/robot/speak")),
            brain_tts_status_endpoint=str(self.cfg.get("brain_tts_status_endpoint", "/robot/tts/status")),
            ...
            audio_noise_reduction_strength=float(self.cfg.get("audio_noise_reduction_strength", 0.45)),
            ...
            stt_min_confidence=float(self.cfg.get("stt_min_confidence", 0.45)),
            stt_min_voiced_ms=int(self.cfg.get("stt_min_voiced_ms", 320)),
            ...
            stt_capture_method=str(self.cfg.get("stt_capture_method", "auto")),
    ...
    def _send_mouth_colour(self, colour: str, brightness: float) -> None:
        ...
        try:
            ...
            self.web_log("error", f"mouth LED command failed: {exc}", {"action": action})
    ...
    def handle_mouth_audio_event(self, event: str, info: Dict[str, Any]) -> None:
        ...
        # a USB webcam microphone near the robot speaker can hear BX1's own
        # Chatterbox reply and feed it back into the wake listener.
        ...
        if event == "speech_prepare":
            # Subtle thinking/generation cue using the user-selected state profile.
    ...
    def start(self) -> None:
        ...
        if bool(self.cfg.get("send_periodic_camera_frames", False)):
    ...
    def read_body_state(self) -> Dict[str, Any]:
        ...
        state["sensors"] = {
            "imu_ok": bool(state.get("mcu_ok", False)) or bool(state.get("imu_ok", False)),
    ...
    def print_banner(self) -> None:
        ...
        print(f" Brain TTS : {tts_url or 'not configured - open /connection and use this browser PC'}")
    ...
    def _match_wake_word(self, text: str, wake_words: List[str]) -> str:
        ...
        for wake in sorted((self._normalise_spoken_text(w) for w in wake_words if str(w).strip()), key=len, reverse=True):
            if wake and re.search(r"(?:^|\s)" + re.escape(wake) + r"(?:$|\s)", low):
                return wake
        return ""
    ...
    def _strip_wake_words(self, text: str, wake_words: List[str]) -> str:
        ...
        for wake in sorted((self._normalise_spoken_text(w) for w in wake_words if str(w).strip()), key=len, reverse=True):
            if wake:
                cleaned = re.sub(r"(?:^|\s)" + re.escape(wake) + r"(?=$|\s)", " ", cleaned)
    ...
    def voice_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            # Chatterbox reply/cue is still queued. This is more reliable than
            ...
            try:
                ...
                try:
                    if monitor_was_running:
                        ...
                        time.sleep(0.1)
                    stt_result = self.stt.listen_once_detailed(float(self.cfg.get("record_seconds", 5)))
            ...
            matched_wake = self._match_wake_word(low, wake_words)
            ...
            if wake_words and not matched_wake and not is_awake_waiting:
                ...
                self.record_input_event("microphone", "wake_gate", text, False, "no wake word while conversation inactive", event_id, metrics)
                continue
            ...
            if matched_wake and not cleaned:
                ...
                if bool(self.cfg.get("wake_voice_ack_enabled", True)):
                    phrase = str(self.cfg.get("wake_ack_phrase", "Yes John?") or "Yes John?")
                    if not self.play_local_voice_cue("wake_ack", phrase, allow_main_tts_fallback=False):
                        # Do not call Brain/Chatterbox live for the wake acknowledgement;
            ...
            if bool(self.cfg.get("voice_command_immediate_cue_enabled", True)):
                self.play_local_voice_cue_async("thinking_01", "I am checking that.", allow_main_tts_fallback=False)
            ...
            if result.get("ok", False):
                hold_s = max(5.0, min(600.0, float(self.cfg.get("conversation_followup_window_s", 45.0))))
                awake_until = time.monotonic() + hold_s
                self.set_conversation_active_until(awake_until)
                ...
                # Chatterbox audio is queued/playing.
    ...
    def _idle_life_micro_action(self) -> None:
        ...
        try:
            ...
            self.web_log("idle", "idle micro action: glance/eye pulse", {"yaw": round(yaw, 2), "pitch": round(pitch, 2)})
    ...
    def _idle_life_speak_phrase(self, phrase: str, cue_index: int = 1) -> None:
        phrase = str(phrase or "").strip()
        if not phrase:
            return
        cue_key = f"idle_chatter_{max(1, min(20, int(cue_index))):02d}"
        self.web_log("idle", f"idle chatter: {phrase}", {"cue_key": cue_key})
    ...
    def handle_idle_curiosity(self) -> Dict[str, Any]:
        """Ask the Brain App for a short autonomous fact/comment.

        Internet access still belongs to the laptop Brain App.  The body client
        only asks for a tiny 'idle curiosity' response and requests web access
        when the setting allows it.
        """
        prompt = str(self.cfg.get("idle_life_curiosity_prompt", "") or "").strip()
        if not prompt:
            prompt = (
                "Autonomous idle curiosity mode. The user has not spoken; do not pretend they asked a question. "
                "As BX1, say one short interesting fact, tiny observation, weather/news-style curiosity, or light robot thought. "
                "Keep it under two sentences, do not be annoying, and do not ask the user to reply."
            )
        state = self.read_body_state()
        self.web_log("idle", "idle curiosity request sent to Brain App", {"use_web": bool(self.cfg.get("idle_life_internet_curiosity_enabled", False))})
        thinking_stop = self.start_thinking_cues("idle")
        t0 = time.perf_counter()
        try:
            result = self.brain.chat(
                ...
                message=prompt,
                body_state=state,
                use_web=bool(self.cfg.get("idle_life_internet_curiosity_enabled", False)),
            ...
            self.web_log("error", f"idle curiosity failed: {exc}")
            self.set_idle_life_runtime("error", last_error=str(exc))
            return {"ok": False, "error": str(exc)}
        finally:
            thinking_stop.set()
        self.touch_robot_activity("idle_curiosity")
        return self.handle_brain_result(result, original_message="idle curiosity")
    ...
    def idle_life_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            self.set_idle_life_runtime("waiting", comments_this_hour=comments_this_hour, sleeping=False)
            ...
            if user_idle_s >= sleep_after:
                ...
                if not self.idle_life_sleep_announced and bool(self.cfg.get("idle_life_sleep_announce_enabled", False)):
                    self.idle_life_sleep_announced = True
                    phrase = str(self.cfg.get("sleep_ack_phrase", "Going quiet.") or "Going quiet.")
                    self.play_local_voice_cue_async("sleep_ack", phrase, allow_main_tts_fallback=False)
                    self.touch_robot_activity("idle_sleep")
                self.stop_event.wait(10.0)
                continue

            micro_delay = max(10.0, float(self.cfg.get("idle_life_micro_action_delay_s", 120.0)))
            ...
            comment_delay = max(30.0, float(self.cfg.get("idle_life_comment_delay_s", 300.0)))
            comment_interval = max(60.0, float(self.cfg.get("idle_life_min_comment_interval_s", 600.0)))
            if (
                bool(self.cfg.get("idle_life_self_chatter_enabled", True))
                and max_comments > 0
                ...
                self._idle_life_speak_phrase(phrases[idx], idx + 1)
                self.stop_event.wait(3.0)
                continue
    ...
    def get_idle_life_settings(self) -> Dict[str, Any]:
        return {
            ...
            "self_chatter_enabled": bool(self.cfg.get("idle_life_self_chatter_enabled", True)),
            "internet_curiosity_enabled": bool(self.cfg.get("idle_life_internet_curiosity_enabled", False)),
            "sleep_announce_enabled": bool(self.cfg.get("idle_life_sleep_announce_enabled", False)),
            "micro_action_delay_s": float(self.cfg.get("idle_life_micro_action_delay_s", 120.0)),
            ...
            "phrases": self._idle_life_phrases(),
            "curiosity_prompt": str(self.cfg.get("idle_life_curiosity_prompt", "")),
    ...
    def web_update_idle_life_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.cfg["idle_life_self_chatter_enabled"] = bool(data.get("self_chatter_enabled", self.cfg.get("idle_life_self_chatter_enabled", True)))
        ...
        self.cfg["idle_life_sleep_announce_enabled"] = bool(data.get("sleep_announce_enabled", self.cfg.get("idle_life_sleep_announce_enabled", False)))
        self.cfg["idle_life_micro_action_delay_s"] = clamp_float(data.get("micro_action_delay_s", self.cfg.get("idle_life_micro_action_delay_s", 120.0)), 10.0, 3600.0, 120.0)
        self.cfg["idle_life_micro_action_interval_s"] = clamp_float(data.get("micro_action_interval_s", self.cfg.get("idle_life_micro_action_interval_s", 90.0)), 20.0, 7200.0, 90.0)
        self.cfg["idle_life_comment_delay_s"] = clamp_float(data.get("comment_delay_s", self.cfg.get("idle_life_comment_delay_s", 300.0)), 30.0, 7200.0, 300.0)
        self.cfg["idle_life_min_comment_interval_s"] = clamp_float(data.get("min_comment_interval_s", self.cfg.get("idle_life_min_comment_interval_s", 600.0)), 60.0, 7200.0, 600.0)
        self.cfg["idle_life_curiosity_delay_s"] = clamp_float(data.get("curiosity_delay_s", self.cfg.get("idle_life_curiosity_delay_s", 900.0)), 60.0, 10800.0, 900.0)
        self.cfg["idle_life_curiosity_interval_s"] = clamp_float(data.get("curiosity_interval_s", self.cfg.get("idle_life_curiosity_interval_s", 1800.0)), 300.0, 21600.0, 1800.0)
        self.cfg["idle_life_sleep_after_s"] = clamp_float(data.get("sleep_after_s", self.cfg.get("idle_life_sleep_after_s", 1800.0)), 60.0, 43200.0, 1800.0)
        self.cfg["idle_life_max_comments_per_hour"] = clamp_int(data.get("max_comments_per_hour", self.cfg.get("idle_life_max_comments_per_hour", 3)), 0, 12, 3)
        self.cfg["idle_life_head_yaw_deg"] = clamp_float(data.get("head_yaw_deg", self.cfg.get("idle_life_head_yaw_deg", 8.0)), 0.0, 30.0, 8.0)
        self.cfg["idle_life_head_pitch_deg"] = clamp_float(data.get("head_pitch_deg", self.cfg.get("idle_life_head_pitch_deg", 4.0)), 0.0, 20.0, 4.0)
        self.cfg["idle_life_eye_colour"] = str(data.get("eye_colour", self.cfg.get("idle_life_eye_colour", "blue")) or "blue").strip()
        self.cfg["idle_life_eye_brightness"] = clamp_float(data.get("eye_brightness", self.cfg.get("idle_life_eye_brightness", 0.14)), 0.0, 1.0, 0.14)
        self.cfg["idle_life_phrases"] = phrases or list(DEFAULT_IDLE_CHATTER_PHRASES)
        self.cfg["idle_life_curiosity_prompt"] = str(data.get("curiosity_prompt", self.cfg.get("idle_life_curiosity_prompt", "")) or "").strip()
        self.save_config_file()
        self.set_idle_life_runtime("waiting", "Idle-life settings saved")
        self.web_log("system", "Idle-life settings saved")
    ...
    def web_test_idle_life_phrase(self) -> Dict[str, Any]:
        ...
        idx = random.randrange(len(phrases))
        self._idle_life_speak_phrase(phrases[idx], idx + 1)
        return {"ok": True, "phrase": phrases[idx], "idle_life": self.get_idle_life_settings()}
    ...
    def _current_personality_controls(self) -> Dict[str, int]:
        personality = dict(self.robot_profile.get("personality") or {})
        controls = normalise_personality_controls(personality.get("controls", {}), personality)
        personality["controls"] = controls
        personality["humour_level"] = float(controls.get("humour", 35)) / 100.0
        personality["confidence_level"] = float(controls.get("confidence", 65)) / 100.0
        self.robot_profile["personality"] = personality
        return controls

    def _save_personality_controls(self, controls: Dict[str, int], reason: str = "voice") -> Dict[str, int]:
        personality = dict(self.robot_profile.get("personality") or {})
        merged = normalise_personality_controls(controls, personality)
        personality["controls"] = merged
        personality["humour_level"] = float(merged.get("humour", 35)) / 100.0
        personality["confidence_level"] = float(merged.get("confidence", 65)) / 100.0
        personality.setdefault("style_strength", 95)
        self.robot_profile["personality"] = personality
        self.apply_robot_profile_to_config(save=True)
        self.web_log("system", f"Robot personality updated from {reason}: {personality_controls_line(merged)}")
        return merged

    def _trait_from_text(self, text: str) -> Optional[str]:
        low = str(text or "").lower()
        for alias, canonical in sorted(PERSONALITY_CONTROL_ALIASES.items(), key=lambda kv: -len(kv[0])):
    ...
    def handle_user_text(self, text: str, use_web: Optional[bool] = None, use_memory: Optional[bool] = None,
        ...
        if self.is_repeat_last_request(text):
            return self.repeat_last_response()
        web_allowed = bool(self.cfg.get("use_web_for_robot_questions", False) if use_web is None else use_web)
        vision_allowed = bool(self.cfg.get("auto_camera_on_vision_request", True) if allow_vision is None else allow_vision)
        local_personality = self.handle_personality_command(text)
        ...
        try:
            result = self.brain.chat(
                ...
                body_state=state,
                use_web=web_allowed,
                use_memory=bool(self.cfg.get("use_memory", True) if use_memory is None else use_memory),
                robot_profile={},
                source=source,
                trigger=trigger,
                event_id=event_id,
        ...
        return self.handle_brain_result(result, original_message=text)
    ...
    def handle_vision(self, prompt: str, use_web: Optional[bool] = None,
        ...
        try:
            ...
            result = self.brain.vision(
                ...
                image_bytes=image_bytes,
                body_state=state,
                use_web=bool(self.cfg.get("use_web_for_robot_questions", False) if use_web is None else use_web),
                robot_profile={},
                source=source,
                trigger=trigger,
                event_id=event_id,
            ...
            self.web_log("error", f"vision failed: {exc}")
            return {"ok": False, "error": str(exc)}
        finally:
            thinking_stop.set()
            self.apply_led_state("awake", source="vision_complete", include_mouth=False, force=True)
        return self.handle_brain_result(result, original_message=prompt)

    def handle_brain_result(self, result: Dict[str, Any], original_message: str) -> Dict[str, Any]:
        if not result.get("ok", False):
            ...
            return {"ok": False, "error": visible, "raw": result}
        ...
        if reply:
            ...
            brain_audio_present = bool(result.get("audio_url") or result.get("audio"))
            ...
            if audio_already_played_on_robot:
                ...
                # Audio metadata from the desktop does not mean the physical robot has played it.
                # The body owns speaker playback so its mouth LED receives real start/stop/envelope events.
                if brain_audio_present:
                    self.web_log("system", "Brain returned audio metadata; body TTS still owns robot playback and mouth animation")
                self.tts.speak(reply, flush=True, tag="reply")
    ...
    def get_wake_words(self) -> List[str]:
        words = [str(w).strip().lower() for w in self.cfg.get("wake_words", ["robot"]) if str(w).strip()]
        return words or ["robot"]
    ...
    def get_local_voice_cue_texts(self) -> Dict[str, str]:
        ...
        # Chatterbox filler phrases match the current robot personality.
    ...
    def play_local_voice_cue(self, cue_key: str, fallback_text: str = "", allow_main_tts_fallback: bool = False) -> bool:
        ...
        if path is not None:
            with self.local_cue_lock:
                ...
                self.handle_mouth_audio_event("speech_audio_file_start", {"text": text, "backend": "local-cue", "filename": str(path), "wav": path.suffix.lower() == ".wav", "audio_profile": {}})
                try:
                    ...
                    self.handle_mouth_audio_event("speech_audio_file_stop", {"text": text, "backend": "local-cue", "filename": str(path)})
        ...
        if bool(self.cfg.get("local_cue_fallback_espeak_enabled", True)) and text:
            ...
            # when the Chatterbox cue cache has not been generated yet.
            try:
    ...
    def web_regenerate_local_voice_cues(self, data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"ok": False, "error": "Brain TTS URL could not be derived from the Brain App URL: " + str(exc)}
        LOCAL_CUE_DIR.mkdir(parents=True, exist_ok=True)
        ...
        manifest = {
            ...
            "voice": str(self.cfg.get("brain_tts_voice", "chatterbox_bx1")),
            "engine": str(self.cfg.get("brain_tts_engine", "chatterbox_turbo")),
            "cues": {},
        }
    ...
    def start_thinking_cues(self, reason: str = "chat") -> threading.Event:
        ...
        if not bool(self.cfg.get("thinking_cues_enabled", True)):
            return stop

        def worker() -> None:
            try:
                delay = max(0.5, float(self.cfg.get("thinking_cue_delay_s", 3.5)))
                repeat = max(1.0, float(self.cfg.get("thinking_cue_repeat_s", 7.0)))
                max_cues = max(0, int(self.cfg.get("thinking_cue_max_per_reply", 2)))
                if max_cues <= 0:
                    return
                if stop.wait(delay):
                    return
                cues = self.get_thinking_cues()
                for idx in range(max_cues):
                    if stop.is_set():
                        return
                    cue = random.choice(cues) if cues else "Hmm."
                    self.web_log("thinking", cue, {"reason": reason, "index": idx + 1})
                    if bool(self.cfg.get("thinking_cue_speak", True)):
                        ...
                        for key, text in self.get_local_voice_cue_texts().items():
                            if str(text).strip() == cue:
                                ...
                                break
                        if not cue_key:
                            cue_key = f"thinking_{(idx % 20) + 1:02d}"
                        used_cache = self.play_local_voice_cue(cue_key, cue, allow_main_tts_fallback=False)
                        if not used_cache and bool(self.cfg.get("thinking_cue_use_main_tts_when_uncached", False)):
                            self.tts.speak(cue, flush=False, tag="thinking")
                    if idx >= max_cues - 1:
                        return
                    if stop.wait(repeat):
                        return
            except Exception as exc:
                self.web_log("error", f"thinking cue failed: {exc}")

        threading.Thread(target=worker, name="bx1-thinking-cues", daemon=True).start()
        return stop
    ...
    def normalise_brain_tts_base_url(self, value: str) -> str:
        """Accept either a full URL or bare IP/hostname and return a clean Brain TTS base URL."""
        raw = str(value or "").strip()
        if not raw:
            raise ValueError("Brain TTS URL/IP cannot be empty.")
        if "://" not in raw:
            raw = "http://" + raw
        parsed = urlparse(raw)
        if parsed.scheme not in {"http", "https"}:
            raise ValueError("Brain TTS URL must start with http:// or https://")
        if not parsed.hostname:
            raise ValueError("Brain TTS URL must include an IP address or hostname.")
    ...
    def derive_brain_tts_base_url(self, port: int = 8091) -> str:
        """Derive the Chatterbox/Brain TTS host from the saved Brain API URL.

        The Brain API normally listens on 8765 and the TTS service on 8091 on
        the same Windows laptop.  Keeping this derived removes a fragile second
        IP field from normal use.
        """
    ...
    def resolve_brain_tts_base_url(self, update_config: bool = True) -> str:
        """Return the active Brain TTS URL, deriving it from Brain App URL by default."""
        ...
        if follow_brain_host or self._is_placeholder_url(current):
            derived = self.derive_brain_tts_base_url(8091)
            if derived:
                if update_config:
    ...
    def sync_brain_tts_to_brain_host(self, save: bool = False) -> Dict[str, Any]:
        """Force Brain TTS to follow the saved Brain App host on port 8091."""
        url = self.derive_brain_tts_base_url(8091)
        if not url:
        ...
        self.cfg["brain_tts_follow_brain_host"] = True
        self.cfg["tts_backend"] = "brain-tts"
        self.cfg["brain_tts_engine"] = str(self.cfg.get("brain_tts_engine", "chatterbox_turbo") or "chatterbox_turbo")
        self.cfg["brain_tts_voice"] = str(self.cfg.get("brain_tts_voice", "chatterbox_bx1") or "chatterbox_bx1")
        self.cfg["brain_tts_endpoint"] = "/robot/speak"
        self.cfg["brain_tts_status_endpoint"] = "/robot/tts/status"
    ...
    def apply_robot_profile_to_config(self, save: bool = False) -> None:
        ...
        self.cfg["wake_words"] = list(profile.get("wake_words", self.cfg.get("wake_words", ["robot"])))
    ...
    def sync_voice_profile_from_audio_config(self) -> None:
        ...
        elif backend == "brain-tts":
            voice["voice"] = str(self.cfg.get("brain_tts_voice", "chatterbox_bx1"))
            voice["brain_tts_base_url"] = str(self.cfg.get("brain_tts_base_url", ""))
            voice["engine"] = "brain-tts"
        else:
    ...
    def get_brain_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "portable_hint": "If you move network, open this page from the Brain PC and press Use This Browser PC for Brain + Voice.",
        }
    ...
    def apply_audio_settings(self, data: Dict[str, Any], save: bool = True) -> Dict[str, Any]:
        ...
        allowed_backends = {"espeak-ng", "piper", "edge-tts", "brain-tts", "custom"}
        backend = str(data.get("tts_backend", self.cfg.get("tts_backend", "espeak-ng"))).strip().lower()
        if backend in {"espeak", "espeakng"}:
            backend = "espeak-ng"
        if backend in {"edge", "edge_tts"}:
            backend = "edge-tts"
        if backend in {"elevenlabs", "eleven-labs", "11labs", "eleven_labs"}:
            backend = "brain-tts"
        if backend in {"brain", "brain_tts", "robot-brain", "robot_brain", "chatterbox"}:
            backend = "brain-tts"
        if backend not in allowed_backends:
            raise ValueError("tts_backend must be one of: espeak-ng, piper, edge-tts, brain-tts, custom")

        self.cfg["tts_enabled"] = bool(data.get("tts_enabled", self.cfg.get("tts_enabled", True)))
        self.cfg["tts_backend"] = backend
        self.cfg["tts_voice"] = str(data.get("tts_voice", self.cfg.get("tts_voice", "en-gb"))).strip() or "en-gb"
        self.cfg["tts_rate"] = clamp_int(data.get("tts_rate", self.cfg.get("tts_rate", 155)), 80, 260, 155)
        self.cfg["tts_pitch"] = clamp_int(data.get("tts_pitch", self.cfg.get("tts_pitch", 35)), 0, 99, 35)
        ...
        follow_brain_host = bool(data.get("brain_tts_follow_brain_host", True if backend == "brain-tts" else self.cfg.get("brain_tts_follow_brain_host", True)))
        self.cfg["brain_tts_follow_brain_host"] = follow_brain_host
        if backend == "brain-tts" and (follow_brain_host or self._is_placeholder_url(requested_tts_url)):
            self.cfg["brain_tts_base_url"] = self.resolve_brain_tts_base_url(update_config=True)
        else:
            self.cfg["brain_tts_base_url"] = self.normalise_brain_tts_base_url(requested_tts_url) if requested_tts_url else ""
        self.cfg["brain_tts_engine"] = str(data.get("brain_tts_engine", self.cfg.get("brain_tts_engine", "chatterbox_turbo"))).strip() or "chatterbox_turbo"
        self.cfg["brain_tts_voice"] = str(data.get("brain_tts_voice", self.cfg.get("brain_tts_voice", "chatterbox_bx1"))).strip() or "chatterbox_bx1"
        ...
        endpoint = str(data.get("brain_tts_endpoint", self.cfg.get("brain_tts_endpoint", "/robot/speak"))).strip() or "/robot/speak"
        status_endpoint = str(data.get("brain_tts_status_endpoint", self.cfg.get("brain_tts_status_endpoint", "/robot/tts/status"))).strip() or "/robot/tts/status"
        ...
        self.cfg["tts_command"] = str(data.get("tts_command", self.cfg.get("tts_command", "espeak-ng -ven-gb -s 155 -p 35"))).strip()
        self.cfg["tts_piper_model"] = str(data.get("tts_piper_model", self.cfg.get("tts_piper_model", "models/piper/en_GB-alan-medium.onnx"))).strip()
        self.cfg["tts_piper_config"] = str(data.get("tts_piper_config", self.cfg.get("tts_piper_config", ""))).strip()
        self.cfg["tts_edge_voice"] = str(data.get("tts_edge_voice", self.cfg.get("tts_edge_voice", "en-GB-SoniaNeural"))).strip() or "en-GB-SoniaNeural"
        ...
        if save:
            self.save_robot_profile_file()
            self.save_config_file()
        self.web_log("system", f"Speech settings saved: backend={backend}, volume={self.cfg['tts_volume']}%")
    ...
    def web_robot_brain_tts_status(self) -> Dict[str, Any]:
        """Check the PC Robot Brain TTS bridge used for Chatterbox robot voice."""
        import urllib.error
        import urllib.request
        ...
        endpoint = str(self.cfg.get("brain_tts_status_endpoint", "/robot/tts/status") or "/robot/tts/status").strip()
        if not endpoint.startswith("/"):
            endpoint = "/" + endpoint
        if not base:
            return {
                "ok": False,
                "error": "Brain TTS URL is not set. Open Brain Connection and press Use This Browser PC for Brain + Voice.",
        ...
        try:
            ...
            msg = "Robot Brain TTS bridge is reachable." if ok else "Robot Brain TTS bridge replied but did not report ok=true."
            ...
            self.web_log("error", "Robot Brain TTS status failed: " + result["error"], result)
            return result
        except Exception as exc:
            result = {"ok": False, "url": url, "error": str(exc), "audio": self.get_audio_settings()}
            self.web_log("error", "Robot Brain TTS status failed: " + str(exc), result)
            return result
    ...
    def web_update_brain_settings(self, data: Dict[str, Any], client_ip: Optional[str] = None) -> Dict[str, Any]:
        ...
        'Use This Browser PC' saves that browser machine as BOTH the Brain API host
        and the Brain TTS host.  This avoids hard-coding old house/workshop IPs.
        """
        ...
        try:
            ...
            if sync_tts:
                tts_port = int(str(data.get("brain_tts_port", "8091") or "8091").strip())
                tts_url = self.make_same_host_url(settings["base_url"], tts_port)
                self.cfg["brain_tts_base_url"] = self.normalise_brain_tts_base_url(tts_url)
                self.cfg["brain_tts_follow_brain_host"] = True
                self.cfg["tts_backend"] = "brain-tts"
                self.cfg["brain_tts_engine"] = str(self.cfg.get("brain_tts_engine", "chatterbox_turbo") or "chatterbox_turbo")
                self.cfg["brain_tts_voice"] = str(self.cfg.get("brain_tts_voice", "chatterbox_bx1") or "chatterbox_bx1")
        ...
        if sync_tts:
            msg += f"; Brain TTS URL saved: {self.cfg.get('brain_tts_base_url')}"
    ...
    def get_chat_bridge_settings(self) -> Dict[str, Any]:
        ...
        for phrase in configured:
            ...
            if low and low not in phrases:
                phrases.append(low)
        return {
            "use_web_for_robot_questions": bool(self.cfg.get("use_web_for_robot_questions", False)),
            "use_memory": bool(self.cfg.get("use_memory", True)),
    ...
    def web_stt_once(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Record one short microphone sample, transcribe it, and optionally send it to the Brain App."""
        send_to_brain = bool(data.get("send", False))
        record_data = {
            "seconds": data.get("seconds", self.cfg.get("record_seconds", 5)),
            "mic_device": data.get("mic_device", self.cfg.get("mic_device", "default")),
            "sample_rate": data.get("sample_rate", self.cfg.get("sample_rate", 16000)),
        }
        self.set_voice_runtime("recording", "Listen Once: recording microphone sample...", loop_active=bool(self.voice_runtime.get("loop_active", False)))
        recording = self.web_record_mic_test(record_data)
        if not recording.get("ok"):
            self.set_voice_runtime("error", "Listen Once failed: microphone recording failed", last_error="microphone recording failed")
            return {"ok": False, "stage": "record", "recording": recording, "error": "microphone recording failed"}
        self.set_voice_runtime("transcribing", "Listen Once: transcribing speech...", loop_active=bool(self.voice_runtime.get("loop_active", False)))
        stt = self.web_stt_mic_test({"vosk_model_path": data.get("vosk_model_path", self.cfg.get("vosk_model_path", "models/vosk-model-small-en-us-0.15"))})
        text = str((stt.get("stt") or {}).get("text") or "").strip()
        if text:
            self.set_voice_runtime("heard", f"Listen Once heard: {text}", last_heard=text)
        response: Optional[Dict[str, Any]] = None
        if send_to_brain:
            if not text:
                self.set_voice_runtime("ignored", "Listen Once did not recognise any speech.", last_ignored="")
                return {"ok": False, "stage": "stt", "recording": recording, "stt": stt, "text": "", "error": "no speech recognised"}
            self.set_voice_runtime("processing", "Listen Once recognised speech. Sending to Brain App...", last_accepted=text)
            with self.command_lock:
                response = self.handle_user_text(
                    text,
        ...
        if send_to_brain:
            if ok:
                self.set_voice_runtime("listening", "Listen Once reply complete. Listening for wake word...", loop_active=bool(self.voice_runtime.get("loop_active", False)))
            else:
                err = str((response or {}).get("error") or "Brain App response failed")
                self.set_voice_runtime("error", f"Listen Once send failed: {err}", last_error=err)
        return {
            "ok": ok,
    ...
    def web_update_identity_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        raw_wake = data.get("wake_words", self.cfg.get("wake_words", ["robot"]))
        self.cfg["wake_words"] = normalise_text_list(raw_wake) or ["robot"]
        ...
        self.robot_profile.update({
            ...
            "robot_id": body_id,
            "brain_owns_identity": True,
            "wake_words": list(self.cfg.get("wake_words", ["robot"])),
    ...
    def web_update_mic_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.cfg["audio_noise_reduction_strength"] = clamp_float(data.get("audio_noise_reduction_strength", self.cfg.get("audio_noise_reduction_strength", 0.45)), 0.0, 1.0, 0.45)
        ...
        self.cfg["stt_min_confidence"] = clamp_float(data.get("stt_min_confidence", self.cfg.get("stt_min_confidence", 0.45)), 0.0, 1.0, 0.45)
        self.cfg["stt_min_voiced_ms"] = clamp_int(data.get("stt_min_voiced_ms", self.cfg.get("stt_min_voiced_ms", 320)), 80, 3000, 320)
    ...
    def get_voice_settings(self) -> Dict[str, Any]:
        return {
            ...
            "stt_error": str(getattr(self.stt, "error", "")) if self.stt is not None else "",
            "runtime": self.get_voice_runtime_snapshot(),
        }
    ...
    def web_update_voice_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.cfg["wake_words"] = wake_words or ["robot"]
        self.robot_profile["wake_words"] = wake_words
        self.save_robot_profile_file()
        self.save_config_file()
    ...
    def get_thinking_cue_settings(self) -> Dict[str, Any]:
        return {
            ...
            "thinking_cue_delay_s": float(self.cfg.get("thinking_cue_delay_s", 2.0)),
            "thinking_cue_repeat_s": float(self.cfg.get("thinking_cue_repeat_s", 8.0)),
            "thinking_cue_max_per_reply": int(self.cfg.get("thinking_cue_max_per_reply", 1)),
            ...
            "voice_feedback_tone_level": float(self.cfg.get("voice_feedback_tone_level", 0.16)),
    ...
    def web_update_thinking_cue_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.cfg["thinking_cue_delay_s"] = clamp_float(data.get("thinking_cue_delay_s", self.cfg.get("thinking_cue_delay_s", 3.5)), 0.5, 30.0, 3.5)
        self.cfg["thinking_cue_repeat_s"] = clamp_float(data.get("thinking_cue_repeat_s", self.cfg.get("thinking_cue_repeat_s", 7.0)), 1.0, 60.0, 7.0)
        self.cfg["thinking_cue_max_per_reply"] = clamp_int(data.get("thinking_cue_max_per_reply", self.cfg.get("thinking_cue_max_per_reply", 1)), 0, 10, 1)
        ...
        self.cfg["voice_feedback_tone_level"] = clamp_float(data.get("voice_feedback_tone_level", self.cfg.get("voice_feedback_tone_level", 0.16)), 0.02, 0.35, 0.16)
        ...
        self.save_config_file()
        self.web_log("system", "Thinking cue settings saved")
