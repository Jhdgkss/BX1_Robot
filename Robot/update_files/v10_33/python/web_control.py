...
<title>BX1 Robot Control 10.33</title>
<style>
:root{
...
<div class="app">
<aside class="sidebar">
  <div class="logo"><div class="eyebrow">BX1 / LEO</div><h1>ROBOT<br>CONTROL</h1><small>Robot client v10.33</small></div>
...
function deriveHealth(d){const s=d.state||{},vr=d.voice_runtime||{},ml=((d.mic_level||{}).level)||{},mouth=d.mouth_audio||{};const brainBad=!!s.brain_error;const sttReady=(d.voice||{}).stt_ready;const liveMic=!!vr.loop_active&&!['disabled','error'].includes(String(vr.state||''));const micGood=ml.running||liveMic;const micState=ml.error?'bad':micGood?'good':'warn';const mcu=!!s.mcu_ok;const imu=!!((s.sensors||{}).imu_ok);const mouthState=mouth.last_ok===false?'bad':mouth.commands>0?'good':'warn';const micLabel=liveMic?(['processing','speechgen'].includes(String(vr.state||''))?'MUTED':vr.state==='speaking'?'SPEAKING':vr.state==='recording'?'VOICE':'LISTENING'):(ml.running?(ml.active?'VOICE':'MONITORING'):'STOPPED');return [statusCard('Brain',brainBad?'FAULT':'CONNECTED',brainBad?'bad':'good'),statusCard('STT',sttReady?'READY':((d.voice||{}).stt_error?'FAULT':'IDLE'),sttReady?'good':(d.voice||{}).stt_error?'bad':'warn'),statusCard('Microphone',micLabel,micState),statusCard('MCU Bridge',mcu?'ONLINE':'OFFLINE',mcu?'good':'bad'),statusCard('IMU',imu?'ONLINE':(mcu?'DEGRADED':'UNKNOWN'),imu?'good':mcu?'warn':'off'),statusCard('Mouth LED',mouth.active?'ANIMATING':mouth.commands>0?'READY':'NO EVIDENCE',mouthState)].join('')}
...
class WebControlServer:
    ...
    def start(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "RobotBodyClient/10.33"
