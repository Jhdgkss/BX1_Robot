...
def load_config() -> Dict[str, Any]:
    ...
    # v10.33: natural conversation with strict noise/repetition rejection,
    # unambiguous voice phases and body-owned low-latency feedback.
    cfg["app_version"] = "10.33"
    cfg["version"] = "10.33"
    ...
    cfg.setdefault("thinking_feedback_delay_s", 0.35)
    cfg.setdefault("thinking_cues_enabled", True)
    cfg.setdefault("thinking_cue_speak", True)
    cfg.setdefault("thinking_cue_delay_s", 1.8)
    cfg.setdefault("thinking_cue_repeat_s", 12.0)
    cfg.setdefault("thinking_cue_max_per_reply", 1)
...
class BX1RobotBodyService:
    ...
    def handle_mouth_audio_event(self, event: str, info: Dict[str, Any]) -> None:
        ...
        if event == "speech_prepare":
            ...
            self.set_voice_runtime(
                "speechgen",
                "Building the voice response; microphone muted.",
        ...
        if event in {"speech_start", "speech_audio_file_start"}:
            ...
            restore = "awake" if previous in {"awake", "processing", "speechgen", "heard"} else ("listening" if loop_active else "idle")
