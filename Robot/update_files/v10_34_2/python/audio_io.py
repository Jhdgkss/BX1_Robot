...
class AudioConfig:
    ...
    #   brain-tts  - PC Brain App TTS service, downloads/plays Chatterbox audio
    ...
    # Chatterbox audio and the UNO Q should only download/play the finished file.
    ...
    brain_tts_engine: str = "chatterbox_turbo"
    brain_tts_voice: str = "chatterbox_bx1"
    ...
    # Robot Brain V1.0.8+ bridge endpoints. Use /robot/speak so the PC
    # service applies robot-safe Chatterbox defaults and returns LAN audio URLs.
    brain_tts_endpoint: str = "/robot/speak"
    brain_tts_status_endpoint: str = "/robot/tts/status"
...
class TextToSpeech:
    ...
    def speak(self, text: str, flush: bool = False, tag: str = "reply") -> None:
        ...
        if bool(getattr(self.cfg, "tts_queue_enabled", True)):
            ...
            if flush:
                ...
                # the wake listener remains muted during Chatterbox generation.
                self._speech_active.set()
            for chunk in chunks:
                try:
    ...
    def _brain_tts_base_url(self) -> str:
        ...
        if not base:
            # Best effort fallback for old configs: same PC as Brain API but port 8091.
    ...
    def _rewrite_local_audio_url(self, audio_url: str, base_url: str) -> str:
        """The PC TTS service may return http://127.0.0.1:8091/audio/...
    ...
    def _request_brain_tts_audio(self, text: str) -> dict:
        ...
        Expected service: Robot Brain / BX1 TTS service on port 8091.
        POST /robot/speak with text, engine, voice, play=false.
        ...
        endpoint = str(getattr(self.cfg, "brain_tts_endpoint", "/robot/speak") or "/robot/speak").strip()
        if not endpoint.startswith("/"):
            endpoint = "/" + endpoint
        # Robot Brain V1.0.8+ exposes /robot/speak specifically for the UNO Q.
        # It applies robot-safe Chatterbox defaults and returns LAN-reachable audio URLs.
        ...
        if not bool(getattr(self.cfg, "brain_tts_use_brain_defaults", True)):
            payload["engine"] = (getattr(self.cfg, "brain_tts_engine", "chatterbox_turbo") or "chatterbox_turbo")
            payload["voice"] = (getattr(self.cfg, "brain_tts_voice", "chatterbox_bx1") or "chatterbox_bx1")
        ...
        try:
            ...
            if not audio_url:
                ...
                report["error"] = "TTS service did not return audio_url/relative_audio_url. Check that Robot Brain V1.0.8+ is running and use /robot/speak."
                return report
            ...
            try:
                ...
                detail = str(exc)
            # Compatibility fallback for an older Brain TTS service that still
            ...
            # know /robot/speak, try /speak once. New builds should not need this.
            if exc.code == 404 and speak_url.endswith("/robot/speak"):
                try:
                    ...
                    setattr(self.cfg, "brain_tts_endpoint", old_endpoint or "/robot/speak")
                    return retry
                except Exception:
                    try:
                        setattr(self.cfg, "brain_tts_endpoint", "/robot/speak")
                    except Exception:
                        pass
            ...
            report["total_generation_download_s"] = round(time.perf_counter() - request_started, 3)
            report["error"] = str(exc)
            print(f"[audio] Brain TTS request failed after {report['total_generation_download_s']:.3f}s: {exc}")
            return report

    def _speak_brain_tts_blocking(self, text: str) -> None:
        report = self._request_brain_tts_audio(text)
        if not report.get("ok"):
            print(f"[audio] Brain TTS failed: {report.get('error') or report}")
            self._fallback_speech(text, str(report.get("error") or "Brain TTS failed"))
            return
        ...
        try:
            ...
            if not player:
                print("[audio] No suitable player found for Brain TTS audio. Install aplay/mpg123/ffmpeg.")
                self._fallback_speech(text, "no suitable player for Brain TTS audio")
                return
            ...
            print(
                ...
                f"generation_download={float(report.get('total_generation_download_s') or 0.0):.3f}s "
            ...
            self._emit_mouth_event("speech_audio_file_start", {
                ...
                "tts_timing": {
                    "service_generation_s": report.get("service_generation_s"),
                    "download_s": report.get("download_s"),
                    "total_generation_download_s": report.get("total_generation_download_s"),
                },
            })
            ...
            if not play.get("ok"):
                print(f"[audio] Brain TTS playback failed: {play}")
                self._fallback_speech(text, str(play.get("error") or play.get("stderr") or "playback failed"))
        finally:
            if filename:
                try:
                    os.unlink(filename)
...
class VoskSpeechToText:
    ...
    def _listen_once_alsa_detailed(self, timeout_s: float) -> dict[str, Any]:
        if not shutil.which("arecord"):
        ...
        if keep_debug:
            raw_name = str(temp_root / "bx1_stt_last_raw.wav")
            clean_name = str(temp_root / "bx1_stt_last_filtered.wav")
            submitted_name = str(temp_root / "bx1_stt_last_submitted.wav")
        else:
    ...
    def _listen_once_sounddevice_detailed(self, timeout_s: float) -> dict[str, Any]:
        ...
        try:
            ...
            decision = self._validate(recognition, voice)
            return {**decision, "capture_method": "sounddevice", "confidence": recognition.get("average_confidence"),
    ...
    def listen_once_detailed(self, timeout_s: Optional[float] = None) -> dict[str, Any]:
        if not self.ready:
        ...
        if method in {"auto", "alsa", "arecord"}:
            result = self._listen_once_alsa_detailed(timeout)
        ...
        result = self._listen_once_sounddevice_detailed(timeout)
        attempts.append(result)
        self.last_result = result
        self.error = str(result.get("error") or "")
        result["attempts"] = [{"method": x.get("capture_method"), "reason": x.get("reason"), "error": x.get("error")} for x in attempts]
...
def _bx1_tts_diagnostics(self: TextToSpeech) -> dict:
    ...
    diag = {
        ...
        "brain_tts_engine": str(getattr(self.cfg, "brain_tts_engine", "chatterbox_turbo") or "chatterbox_turbo"),
        "brain_tts_voice": str(getattr(self.cfg, "brain_tts_voice", "chatterbox_bx1") or "chatterbox_bx1"),
        "brain_tts_endpoint": str(getattr(self.cfg, "brain_tts_endpoint", "/robot/speak") or "/robot/speak"),
        "brain_tts_status_endpoint": str(getattr(self.cfg, "brain_tts_status_endpoint", "/robot/tts/status") or "/robot/tts/status"),
    ...
    if backend in {"brain-tts", "brain_tts", "brain", "robot-brain", "robot_brain"}:
        if not str(getattr(self.cfg, "brain_tts_base_url", "") or "").strip():
            problems.append("Brain TTS URL is not set. Use http://YOUR_PC_IP:8091")
...
def _bx1_test_speech_blocking(self: TextToSpeech, text: str) -> dict:
    ...
    playback-stop events, including a real WAV level profile for Brain TTS.
    """
    ...
    try:
        ...
        if backend in {"brain-tts", "brain_tts", "brain", "robot-brain", "robot_brain"}:
            report["voice"] = getattr(self.cfg, "brain_tts_voice", "chatterbox_bx1")
            tts_report = self._request_brain_tts_audio(text)
            report["brain_tts"] = tts_report
            if not tts_report.get("ok"):
                report["message"] = "Brain TTS service did not generate audio: " + str(tts_report.get("error") or tts_report)
                return report
            ...
            try:
                ...
                if not player:
                    report["message"] = "Brain TTS generated audio, but no local player was found."
                    return report
                profile = _bx1_build_wav_mouth_profile(filename) if wav else {"ok": False, "levels": [], "frame_s": 0.055, "duration_s": 0.0, "source": filename}
                ...
                report["ok"] = bool(play.get("ok"))
                report["message"] = "Brain App Chatterbox audio generated on PC and played on BX1 with mouth envelope." if report["ok"] else "Brain TTS generated audio, but playback failed."
                return report
            finally:
                if not event_closed:
                    event_closed = True
