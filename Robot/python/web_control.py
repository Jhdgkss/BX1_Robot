...
<header class="topbar">
  ...
  <div class="card" style="margin-top:10px"><div class="fields three"><div><label>Microphone level</label><div id="audioMicLevel" class="metric">--</div></div><div><label>Gate / wake state</label><div id="audioGateState" class="metric">--</div></div><div><label>Speaker volume</label><input id="audioVolume" type="range" min="0" max="100" oninput="updateAudioControl({tts_volume:Number(this.value)})"><span id="audioVolumeState" class="muted">Unconfirmed</span></div></div><div class="buttons"><button onclick="updateAudioControl({microphone_privacy_muted:true})">Mute Microphone</button><button onclick="updateAudioControl({microphone_privacy_muted:false})">Unmute Microphone</button><button onclick="updateAudioControl({listening_paused:true})">Pause Listening</button><button onclick="updateAudioControl({listening_paused:false})">Resume Listening</button><button onclick="updateAudioControl({speaker_muted:true})">Mute Speaker</button><button onclick="apiAction('/api/stop_speaking',{},this)">Stop Speaking</button></div></div>
</header>
<main>
...
async function updateAudioControl(payload){try{const d=await req('/api/audio_controls',payload);renderAudioControls(d.controls||d);toast('Audio control updated','good')}catch(e){await refreshAll(true);toast(e.message,'bad')}}
function renderAudioControls(c){c=c||{};const a=$('audioGateState');if(a)a.textContent=(c.microphone_gate_closed?'GATED':'OPEN')+' · '+(c.wake_state||'idle');const v=$('audioVolume');const sc=c.speaker_controller||{};if(v&&document.activeElement!==v)v.value=Number(sc.effective_volume_percent??c.speaker_volume??80);const vs=$('audioVolumeState');if(vs)vs.textContent=sc.apply_ok===false?(sc.last_error||'Volume unavailable'):'Confirmed '+Number(sc.effective_volume_percent??c.speaker_volume??80)+'%';const m=$('audioMicLevel');if(m)m.textContent=c.microphone_state||'--'}
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                try:
                    ...
                    if path == "/api/loopback/test":
                        ...
                        self._json(202 if result.get("ok") and result.get("state") == "preparing" else (200 if result.get("ok") else 400), result); return
