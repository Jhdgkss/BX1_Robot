...
TOUCHSCREEN_HTML_PATH = Path(__file__).with_name("touchscreen_display.html")
...
<title>BX1 Robot Control 10.42</title>
<style>
:root{
...
*{box-sizing:border-box}html,body{height:100%;width:100%;max-width:100vw;overflow-x:hidden}body{margin:0;background:radial-gradient(circle at 10% 0,#183a56 0,#08131d 37%,#04080c 100%);color:var(--text);font:15px/1.45 "Segoe UI",Arial,sans-serif}
button,input,select,textarea{font:inherit}button{cursor:pointer}.app{width:100%;max-width:100vw;min-width:0;min-height:100%;display:grid;grid-template-columns:250px minmax(0,1fr);overflow-x:hidden}
...
.workspace{width:100%;max-width:100%;min-width:0;overflow-x:hidden}.topbar{position:sticky;top:0;z-index:10;background:rgba(6,14,21,.94);backdrop-filter:blur(12px);border-bottom:1px solid var(--line);padding:13px 20px}
.topline{display:flex;justify-content:space-between;gap:20px;align-items:center;min-width:0}.topline h2{margin:0;font-size:19px}.topline .meta{min-width:0;max-width:58%;color:var(--muted);font-size:12px;text-align:right}.connection-meta{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:10px;margin-top:2px}.clock{color:var(--cyan);font-size:18px;font-variant-numeric:tabular-nums;font-weight:800}
...
main{width:100%;min-width:0;padding:20px;max-width:1900px;margin:0 auto;overflow-x:hidden}.page{display:none}.page.active{display:block}.page-title{display:flex;align-items:flex-end;justify-content:space-between;gap:15px;margin-bottom:14px}.page-title h3{font-size:25px;margin:0}.page-title p{margin:3px 0 0;color:var(--muted)}
...
label{display:block;color:var(--muted);font-size:12px;margin-bottom:5px}.field{margin:9px 0}.fields{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px}.fields.three{grid-template-columns:repeat(3,minmax(0,1fr))}input,select,textarea{width:100%;color:var(--text);background:#07121b;border:1px solid #334c62;border-radius:8px;padding:9px}input[type=checkbox]{width:auto;margin-right:6px}textarea{min-height:100px;resize:vertical}input[readonly]{opacity:.75}
...
<div class="app">
<aside class="sidebar">
  <div class="logo"><div class="eyebrow">BX1 / LEO</div><h1>ROBOT<br>CONTROL</h1><small>Robot client v10.42</small></div>
...
<div class="workspace">
<header class="topbar">
  <div class="topline"><div><h2 id="headerTitle">Overview</h2><div class="muted" id="headerSubtitle">Body health and active faults</div></div><div class="meta"><div class="clock" id="clock">--:--:--</div><div id="lastRefresh">Waiting for status…</div><div id="connectionMeta" class="connection-meta">Body detecting… · Brain not configured</div></div></div>
...
<section class="page" id="page-speech">
  ...
  <div class="grid">
    ...
    <article class="card span-5"><h4>Wake / Session Status</h4><div id="voiceMetrics" class="metric-grid"></div><div class="field"><label>Wake phrases — supplied by the active Brain profile</label><textarea id="wakeWords" readonly placeholder="Connect the desktop Brain app to synchronise the robot name and wake phrases."></textarea></div><div class="help" id="brainProfileWakeState">Brain app owns the robot name, wake phrases and queued speech definitions.</div><div class="field"><label><input id="voiceEnabled" type="checkbox"> Start listening automatically and keep the live wake-word listener enabled</label></div><div class="buttons"><button class="good" onclick="saveVoice(this)">Save and activate listening</button><button onclick="syncBrainProfile(this)">Sync Brain profile now</button></div><div class="help" id="voiceSaveState">Changes are saved to the robot and survive restart.</div><div class="rule"></div><h5>Speech recognition</h5><div class="field"><label>Local Vosk fallback model</label><input id="voskModelPath" readonly></div><div id="speechModelHint" class="result-box"></div><div class="rule"></div><h5>Speaker output</h5><div class="field"><label>Playback device</label><select id="playbackDevice"></select></div><div class="field"><label>Robot playback volume</label><input id="ttsVolume" type="number" min="0" max="100"></div><button onclick="saveOutput(this)">Save speaker output</button><div class="help" id="voiceOwnerText"></div></article>
  </div>
</section>
...
<section class="page" id="page-advanced">
  ...
  <div class="grid">
    <article class="card span-6"><h4>Brain Connection</h4>
      <div class="fields">
        <div class="field"><label>Robot Body address</label><input id="bodyCurrent" readonly></div>
        <div class="field"><label>Current Brain endpoint</label><input id="brainCurrent" readonly></div>
      </div>
      <div class="fields three">
        <div class="field"><label>Protocol</label><select id="brainScheme"><option value="http">http</option><option value="https">https</option></select></div>
        <div class="field"><label>Brain IP address or hostname</label><input id="brainHost" inputmode="url" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="192.168.68.52"></div>
        <div class="field"><label>Brain API port</label><input id="brainPort" type="number" inputmode="numeric" min="1" max="65535" step="1" value="8765"></div>
      </div>
      <div class="help" id="brainConnectionHint">The Brain API and queued speech use this same host and port.</div>
      <div class="buttons"><button class="primary" onclick="saveBrain(false,this)">Save connection</button><button onclick="saveBrain(true,this)">Use this browser PC</button><button class="good" onclick="testBrain(this)">Test Brain</button></div><pre id="brainResult"></pre>
    </article>
...
function renderSpeech(d){
  ...
  const bp=d.brain_profile||{};if($('brainProfileWakeState'))$('brainProfileWakeState').textContent=`Brain-owned name: ${bp.robot_name||'not synced'} · Source: ${bp.source||'none'} · Last sync: ${bp.last_sync_at||bp.last_cache_load_at||'never'}`;
...
function renderAdvanced(d){const b=d.brain||{},n=d.network||{};const scheme=$('brainScheme'),host=$('brainHost'),port=$('brainPort');if(!isDirty(scheme))scheme.value=b.scheme||n.brain_scheme||'http';if(!isDirty(host))host.value=b.host||n.brain_host||'';if(!isDirty(port))port.value=String(b.port||n.brain_port||8765);$('brainCurrent').value=b.endpoint||b.base_url||n.brain_endpoint||'Not configured';$('bodyCurrent').value=n.body_web_endpoint||((n.primary_ip&&n.body_web_port)?`http://${n.primary_ip}:${n.body_web_port}`:'Detecting…');$('brainConnectionHint').textContent=b.portable_hint||'The Brain API and queued speech use this same host and port.';$('rawSnapshot').textContent=JSON.stringify(d,null,2)}
function renderAll(d){SNAP=d;EVENTS=d.events||[];renderOverview(d);renderConversation(d);renderSpeech(d);renderVision(d);renderIdle(d);renderMouth(d);renderHardware(d);renderDoctor(d);renderAdvanced(d);renderLogs();$('lastRefresh').textContent='Updated '+new Date().toLocaleTimeString();const n=d.network||{};const body=(n.body_web_endpoint||'detecting').replace(/^https?:\/\//,'');const brain=(n.brain_endpoint||'not configured').replace(/^https?:\/\//,'');$('connectionMeta').textContent=`Body ${body} · Brain ${brain}`;$('connectionMeta').title=`Body ${n.body_web_endpoint||''} | Brain ${n.brain_endpoint||'not configured'}`}
...
async function saveVoice(btn){busy(btn,true,'Saving…');try{const v=SNAP.voice||{};await req('/api/voice_settings',{...v,voice_enabled:$('voiceEnabled').checked,input_mode:$('voiceEnabled').checked?'both':'keyboard'});clearDirty($('page-speech'));$('voiceSaveState').textContent='Saved. Active wake phrases remain controlled by the Brain profile.';toast('Listening setting saved','good');await refreshAll(true)}catch(e){toast(e.message,'bad')}finally{busy(btn,false)}}
async function syncBrainProfile(btn){busy(btn,true,'Syncing…');try{const d=await req('/api/sync_brain_profile',{});if(!d.ok)throw new Error(((d.sync||{}).error)||d.error||'Brain profile sync failed');toast('Brain profile and queued speech synchronised','good');await refreshAll(true)}catch(e){toast(e.message,'bad')}finally{busy(btn,false)}}
...
async function saveBrain(useIp,btn){busy(btn,true,'Saving…');try{const port=Number($('brainPort').value);if(!Number.isInteger(port)||port<1||port>65535)throw new Error('Brain port must be a whole number between 1 and 65535.');const payload={brain_scheme:$('brainScheme').value||'http',brain_host:$('brainHost').value.trim(),brain_port:port,use_browser_client_ip:useIp,sync_tts_to_brain_host:true};const d=await req('/api/brain_settings',payload);$('brainResult').textContent=JSON.stringify(d,null,2);clearDirty($('page-advanced'));toast('Brain connection saved: '+((d.brain||{}).endpoint||(d.brain||{}).base_url||''),'good');await refreshAll(true)}catch(e){toast(e.message,'bad')}finally{busy(btn,false)}}
...
class WebControlServer:
    ...
    def start(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            server_version = "RobotBodyClient/10.42"
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                path = parsed.path
                if path in {"/display", "/screen", "/touchscreen"}:
                    try:
                        body = TOUCHSCREEN_HTML_PATH.read_bytes()
                        self._send(200, body, "text/html; charset=utf-8")
                    except Exception as exc:
                        self._send(503, ("Touchscreen display unavailable: " + str(exc)).encode("utf-8"), "text/plain; charset=utf-8")
                    return
                if path in {"/", "/index.html", "/overview", "/conversation", "/speech", "/vision", "/mouth", "/hardware", "/doctor", "/logs", "/advanced", "/chat", "/identity", "/connection", "/microphone", "/cues", "/idle", "/context", "/actions", "/lighting", "/performance", "/telemetry"}:
                    ...
                    return
                if path == "/api/touchscreen":
                    self._json(200, service.web_touchscreen_snapshot())
                    return
                if path == "/api/status":
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                try:
                    ...
                    if path == "/api/thinking_cue_settings":
                        ...
                        self._json(200 if result.get("ok") else 400, result)
                        return
                    if path == "/api/sync_brain_profile":
                        result = service.web_sync_brain_profile()
