...
const fallbackData = {
  interface: {
    ...
    version: "0.10.2-stt-loopback",
    tag: "BX1_OS_v0.10.2_stt_loopback",
  ...
  deployment: {
    current_version: "0.10.2-stt-loopback",
...
function wakeSpeechSettingsPanel(bridge) {
  ...
  const row = phrase => `<div class="wake-phrase-row"><input class="input" name="wake_phrase" maxlength="48" value="${esc(phrase)}" aria-label="Wake phrase"><button class="button small" type="button" data-wake-action="remove">Remove</button></div>`;
  return `<form id="wakeSpeechForm" class="audio-settings wake-speech-form">
    ...
    <div class="wake-phrase-section"><div id="wakePhraseRows" class="wake-phrase-list">${phrases.map(row).join("")}</div><button class="button small wake-add-button" type="button" data-wake-action="add">Add phrase</button></div>
    <div class="wake-controls"><label>Wake/session listening timeout (s)<input class="input" name="wake_listen_timeout_s" type="number" min="3" max="60" step="0.5" value="${esc(value.wake_listen_timeout_s)}"></label>
    ...
    <label>STT policy<select class="input" name="stt_policy"><option value="brain_faster_whisper" ${value.stt_policy === "brain_faster_whisper" ? "selected" : ""}>Brain faster-whisper with local Vosk fallback</option><option value="vosk" ${value.stt_policy === "vosk" ? "selected" : ""}>Local Vosk</option></select></label></div>
    <div class="wake-save-row"><button class="button primary" type="submit">Save to Robot</button><span id="wakeSpeechState" class="mono">Only allowlisted voice settings are saved.</span></div>
  </form>`;
}
...
async function loadAudioBridge() {
  ...
  try {
    const [response, consoleResponse, volumeResponse] = await Promise.all([fetch("/api/audio/bridge", { cache: "no-store" }), fetch("/api/voice/console", { cache: "no-store" }), fetch("/api/audio/volume", { cache: "no-store" })]);
    const [payload, consolePayload, volumePayload] = await Promise.all([response.json(), consoleResponse.json(), volumeResponse.json()]);
    if (response.ok && consoleResponse.ok) {
      state.data.audio_bridge = payload; state.data.voice_console = consolePayload; state.data.audio_volume = volumePayload;
...
async function loadCoreTelemetry() {
  ...
  try {
    ...
    const [coreState, services, health, , , hardware, audio, robotBody, camera, voice, modules, widgets, audioBridge, audioVolume, speechScan, voiceConsole, speechLearning] = await Promise.all(
    ...
    state.data.audio_volume = audioVolume;
...
function canonicalTopCards(data) { const bridge=data.audio_bridge||{}, a=bridge.audio||{}, r=bridge.recognition||{}, brain=data.voice?.brain?.state||"unavailable"; const gate=a.speaker_playback_active?"closed · speaker active":Number(a.echo_tail_remaining_s||0)>0?"closed · echo tail":a.gate_open===false?"closed":"open"; const engine=r.handoff?.engine_selected||r.engine||"unavailable"; const wake=data.voice?.voice_settings?.effective?.wake_phrases||["configured wake phrases"]; const volumeState=data.audio_volume||{}, effective=Number(volumeState.effective_volume_percent), requested=Number(volumeState.requested_volume_percent), volume=Number.isFinite(effective)?effective:(Number.isFinite(requested)?requested:null), output=volumeState.output||a.output||data.audio?.output||{}, outputRms=output.output_rms_dbfs??output.rms_dbfs, outputPeak=output.output_peak_dbfs??output.peak_dbfs, outputActive=output.output_active??a.speaker_playback_active, applyOk=volumeState.apply_ok!==false, unavailableReason=volumeState.last_error||(!volumeState.ok?"Robot Body volume telemetry unavailable":""); return `<div class="canonical-top-cards"><section class="canonical-card"><h4>Microphone</h4>${microphoneGauge(a)}<button class="button danger" data-audio-control="mic-mute">Mute Microphone</button><button class="button" data-audio-control="ptt">Push to Talk</button></section><section class="canonical-card"><h4>Listening / Wake Word</h4><div class="waveform"><i></i><i></i><i></i><i></i><i></i></div><strong>${esc(a.pipeline_state||a.state||"unavailable")}</strong><span>${esc(wake.join(", "))}</span><button class="button" data-audio-control="pause">Pause Listening</button></section><section class="canonical-card speaker-card"><h4>Speaker</h4><strong>${a.speaker_playback_active?"active":"idle"}</strong><span>Requested ${Number.isFinite(requested)?requested+"%":"unavailable"} · Effective ${Number.isFinite(effective)?effective+"%":"unavailable"}</span><input id="managementSpeakerVolume" data-audio-volume="1" type="range" min="0" max="100" value="${Number.isFinite(volume)?volume:0}" aria-label="Speaker volume" ${(!volumeState.ok||!applyOk)?"disabled":""}><small class="speaker-volume-status" role="status">${!volumeState.ok?"Unavailable":(!applyOk?`Failed: ${esc(unavailableReason)}`:`${Number.isFinite(effective)?"Confirmed":"Pending"} ${Number.isFinite(effective)?effective+"%":""}`)}</small><span>Device ${esc(volumeState.physical_playback_device||"unavailable")} · Mixer ${esc(volumeState.mixer_control||"unavailable")}</span><span>Apply ${volumeState.apply_ok===true?"OK":(volumeState.apply_ok===false?"failed":"unavailable")} · ${esc(unavailableReason||"No error")}</span><span>Output RMS ${Number.isFinite(Number(outputRms))?Number(outputRms).toFixed(1)+" dBFS":"unavailable"} · Peak ${Number.isFinite(Number(outputPeak))?Number(outputPeak).toFixed(1)+" dBFS":"unavailable"} · ${outputActive?"active":"idle"}</span><button class="button danger" data-audio-control="speaker-mute">${volumeState.muted?"Unmute Speaker":"Mute Speaker"}</button><button class="button" data-audio-control="stop">Stop Speaking</button></section><section class="canonical-card"><h4>Voice / System State</h4><strong>Gate ${esc(gate)}</strong><span>Brain ${esc(brain)}</span><span>STT ${esc(engine)}</span><span>${esc(a.timestamp?formatTimestamp(a.timestamp):"stale")}</span></section></div>`; }
...
async function loopbackAction(action) { const meta=$("#loopbackMeta"); const buttons=$$('[data-loopback-action]'); if(action==="start"){ if(state.loopbackBusy)return; state.loopbackBusy=true; buttons.forEach(b=>b.disabled=true); if(meta)meta.querySelector("dd").textContent="preparing"; try { const body={expected_phrase:$("#loopbackPhrase")?.value||"",speaker_volume:Number($("#loopbackVolume")?.value||50),post_roll_seconds:Number($("#loopbackPostroll")?.value||1)}; const response=await fetch("/api/audio/loopback/test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}); const payload=await response.json(); if(!response.ok||!payload.ok)throw new Error([payload.error_code,payload.error,payload.field?`(${payload.field})`:""].filter(Boolean).join(" ")||"Loopback test failed"); state.loopbackId=payload.capture_id||""; if(meta){const rows=meta.querySelectorAll("dd"); if(rows[0])rows[0].textContent=payload.state||"completed"; if(rows[1])rows[1].textContent=payload.expected_phrase||body.expected_phrase; if(rows[2])rows[2].textContent=payload.recognised_phrase||"No recognised phrase";} } catch(error){if(meta)meta.querySelector("dd").textContent=`failed · ${error.message}`;} finally {state.loopbackBusy=false; buttons.forEach(b=>b.disabled=false);} return; } if(action==="cancel"){await fetch("/api/audio/loopback/cancel",{method:"POST"}); if(meta)meta.querySelector("dd").textContent="cancelled"; return;} if(action==="play"){const audio=new Audio(`/api/audio/loopback/audio?capture_id=${encodeURIComponent(state.loopbackId||"")}&rev=${Date.now()}`); try{await audio.play();}catch(error){if(meta)meta.querySelector("dd").textContent=`playback failed · ${error.message}`;} return;} if(action==="download"){window.open(`/api/audio/loopback/download?capture_id=${encodeURIComponent(state.loopbackId||"")}&rev=${Date.now()}`,"_blank");return;} const response=await fetch(`/api/audio/loopback/${action}`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}); const payload=await response.json(); if(meta&&payload.state)meta.querySelector("dd").textContent=payload.state; }
...
document.addEventListener("click",event=>{const control=event.target.closest("[data-audio-control]"); if(control){const action=control.dataset.audioControl; control.classList.toggle("active"); const enabled=control.classList.contains("active"); const settings={"mic-mute":{microphone_privacy_muted:enabled},pause:{listening_paused:enabled},"speaker-mute":{speaker_muted:enabled}}[action]; if(settings) fetch("/api/audio/bridge/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({settings})}).then(()=>loadAudioBridge()); if(action==="ptt"){control.classList.add("active");setTimeout(()=>control.classList.remove("active"),8000);} if(action==="stop") fetch("/api/voice/stop-speaking",{method:"POST"}).then(()=>loadAudioBridge());}
 ...
 const loop=event.target.closest("[data-loopback-action]"); if(loop) loopbackAction(loop.dataset.loopbackAction);});
document.addEventListener("input", async event => { const slider=event.target.closest("[data-audio-volume]"); if(!slider || slider.dataset.saving==="1") return; slider.dataset.pendingValue=slider.value; const status=slider.parentElement?.querySelector(".speaker-volume-status"); if(status) status.textContent=`Saving ${slider.value}%…`; if(!slider._volumeCommitTimer) slider._volumeCommitTimer=window.setTimeout(async()=>{ slider._volumeCommitTimer=null; slider.dataset.saving="1"; const requested=Number(slider.dataset.pendingValue||slider.value); try { const response=await fetch("/api/audio/volume",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({volume:requested})}); const payload=await response.json(); if(!response.ok||payload.ok===false) throw new Error(payload.error||payload.last_error||"Speaker volume update failed"); const effective=Number(payload.effective_volume_percent); if(Number.isFinite(effective)) slider.value=String(effective); if(status) status.textContent=`Saved ${Number.isFinite(effective)?effective:requested}%`; state.data.audio_volume=payload; } catch(error) { if(status) status.textContent=`Failed: ${error.message}`; await loadAudioBridge(); } finally { slider.dataset.saving="0"; } },220); });
// Async loopback polling is layered over the legacy action implementation so
// the Body request returns immediately and the current capture ID is retained.
const bx1LegacyLoopbackAction=loopbackAction;
loopbackAction=async function(action){
  const meta=$("#loopbackMeta"), buttons=$$('[data-loopback-action]');
  const render=p=>{if(!meta)return;const rows=meta.querySelectorAll("dd");if(rows[0])rows[0].textContent=p.state||p.stage||"unavailable";if(rows[1])rows[1].textContent=p.expected_phrase||state.loopbackExpected||"Awaiting test";if(rows[2])rows[2].textContent=p.recognised_phrase||p.error||"No recognised phrase";};
  if(action!=="start") return bx1LegacyLoopbackAction(action);
  if(state.loopbackBusy)return; state.loopbackBusy=true; buttons.forEach(b=>b.disabled=true);
  try{const body={expected_phrase:$("#loopbackPhrase")?.value||"",speaker_volume:Number($("#loopbackVolume")?.value||50),post_roll_seconds:Number($("#loopbackPostroll")?.value||1)};const response=await fetch("/api/audio/loopback/test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const payload=await response.json();if(!response.ok||!payload.ok)throw new Error([payload.error_code,payload.error,payload.field?`(${payload.field})`:""].filter(Boolean).join(" ")||"Loopback test failed");state.loopbackId=payload.capture_id||"";state.loopbackExpected=payload.expected_phrase||body.expected_phrase;render(payload);state.loopbackPoll=window.setInterval(async()=>{try{const sr=await fetch(`/api/audio/loopback/status?capture_id=${encodeURIComponent(state.loopbackId)}&rev=${Date.now()}`,{cache:"no-store"}),sp=await sr.json(),v=sp.loopback||sp;render(v);if(["completed","stt_unavailable","cancelled","failed"].includes(v.state)){clearInterval(state.loopbackPoll);state.loopbackPoll=null;state.loopbackBusy=false;buttons.forEach(b=>b.disabled=false);}}catch(e){render({state:"failed",error:e.message});}},500);}catch(e){state.loopbackBusy=false;buttons.forEach(b=>b.disabled=false);render({state:"failed",error:e.message});}
};
const bx1AsyncLoopback=loopbackAction;
loopbackAction=async function(action){
  if(action==="cancel"){
    await fetch("/api/audio/loopback/cancel",{method:"POST"});
    if(state.loopbackPoll){clearInterval(state.loopbackPoll);state.loopbackPoll=null;}
    state.loopbackBusy=false;
    $$('[data-loopback-action]').forEach(b=>b.disabled=false);
    const meta=$("#loopbackMeta"); if(meta)meta.querySelector("dd").textContent="cancelled";
    return;
  }
  return bx1AsyncLoopback(action);
};
