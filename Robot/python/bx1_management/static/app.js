...
const fallbackData = {
  ...
  deployment: {
    current_version: "0.10.1-audio-runtime",
...
function managementDataFromCore(statePayload, servicesPayload, healthPayload, hardwarePayload, audioPayload, robotBodyPayload, cameraPayload, voicePayload = {}, modulesPayload = {}, widgetsPayload = {}, audioBridgePayload = {}, voiceConsolePayload = {}) {
  ...
  return {
    interface: {
      ...
      version: "0.10.1-audio-runtime",
      tag: "BX1_OS_v0.10.1_audio_runtime",
      architecture_only: true,
...
function manualRecordingFrame(){return `<section class="canonical-panel recording-frame"><header><div><h2>Manual Microphone Recording</h2><p>Diagnostic WAV capture only; never submitted as a normal command.</p></div><a class="help-link" href="/documentation#manual-recording">Help</a></header><div class="recording-controls"><label>Duration<select id="manualRecordDuration"><option value="5">5 seconds</option><option value="10">10 seconds</option><option value="20">20 seconds</option><option value="manual">Manual</option></select></label><button class="button primary" data-record-action="start">Start Recording</button><button class="button" data-record-action="stop">Stop Recording</button><button class="button" data-record-action="play">Play Recording</button><button class="button" data-record-action="download">Download WAV</button><button class="button" data-record-action="stt">Send to STT</button><button class="button" data-record-action="save">Save to Speech Learning</button><button class="button danger" data-record-action="delete">Delete Recording</button></div><dl class="recording-meta" id="manualRecordingMeta"><div><dt>State</dt><dd>ready</dd></div><div><dt>Recording ID</dt><dd>none</dd></div><div><dt>Format</dt><dd>WAV · 16 kHz · mono PCM</dd></div><div><dt>Brain</dt><dd>Recording works while Brain is disconnected</dd></div></dl></section>`;}
...
function audioDiagnosticsPage(data){return `<div class="canonical-dashboard linear-audio-layout">${audioStatusFrame(data)}${manualRecordingFrame()}${loopbackFrame()}${canonicalDiagnostics(data)}${panel("Audio-event timeline",sharedVoiceFeed(data,true),{span:12})}${panel("Storage and retention",dataList([["Recording state","Body-owned diagnostic WAV"],["Retention","Configured by Robot Body"],["Storage","Reported by Body sample metadata"],["Brain disconnected","WAV remains available for later STT"]]),{span:12})}<details class="system-overview"><summary>Contained transcription JSON</summary><div class="json-viewer"><pre>${esc(JSON.stringify(data.speech_scan||{},null,2))}</pre></div></details></div>`;}
Object.assign(RENDERERS,{dashboard:dashboardPage,voice:voiceConsolePage,audio:audioDiagnosticsPage,themes:themesPage});
...
function startHeaderClock(){const tick=()=>{const node=$("#bx1Clock");if(!node)return;const now=new Date();node.textContent=new Intl.DateTimeFormat(undefined,{dateStyle:"medium",timeStyle:"medium",timeZoneName:"short"}).format(now);};tick();setInterval(tick,1000);}
...
document.addEventListener("click",event=>{const control=event.target.closest("[data-audio-control]"); if(control){const action=control.dataset.audioControl; control.classList.toggle("active"); const enabled=control.classList.contains("active"); const settings={"mic-mute":{microphone_privacy_muted:enabled},pause:{listening_paused:enabled},"speaker-mute":{speaker_muted:enabled}}[action]; if(settings) fetch("/api/audio/bridge/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({settings})}).then(()=>loadAudioBridge()); if(action==="ptt"){control.classList.add("active");setTimeout(()=>control.classList.remove("active"),8000);} if(action==="stop") fetch("/api/voice/stop-speaking",{method:"POST"}).then(()=>loadAudioBridge());}
 const rec=event.target.closest("[data-record-action]"); if(rec){const action=rec.dataset.recordAction; const meta=$("#manualRecordingMeta"); if(meta) meta.querySelector("dd").textContent=action==="start"?"recording":action==="stop"?"complete":action==="stt"?"waiting for Brain":action; if(action==="start") fetch("/api/audio/recordings/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({seconds:Number($("#manualRecordDuration")?.value||5)})}).then(()=>loadAudioBridge()); if(action==="stop") fetch("/api/audio/recordings/stop",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}); if(action==="play") fetch("/api/audio/recordings/play",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}); if(action==="download") window.open("/api/audio/recordings/download","_blank"); if(action==="stt") fetch("/api/audio/recordings/stt",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}).then(()=>loadAudioBridge());}
 const loop=event.target.closest("[data-loopback-action]"); if(loop){const action=loop.dataset.loopbackAction; const meta=$("#loopbackMeta"); if(meta) meta.querySelector("dd").textContent=action==="start"?"preparing":action; if(action==="start"||action==="stt") fetch("/api/audio/speech-test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({loopback:true,diagnostic:true,phrase:$("#loopbackPhrase")?.value||""})}); if(action==="play") fetch("/api/audio/recordings/play",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}); if(action==="download") window.open("/api/audio/recordings/download","_blank");}});
