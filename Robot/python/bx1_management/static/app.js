...
function dashboardPage(data) { return `<div class="canonical-dashboard">${canonicalTopCards(data)}<div class="canonical-main"><div class="canonical-left">${canonicalConversation(data)}</div><div class="canonical-right">${canonicalScan(data)}${canonicalDiagnostics(data)}</div></div><details class="system-overview"><summary>System Overview</summary>${panel("BX1 OS components",dataList([["Management version",data.interface.version],["Robot Body version",data.robot_body?.version||"unavailable"],["Brain version",data.brain?.version||"unavailable"],["Brain connection",data.brain?.status||"unavailable"]]),{span:12})}</details></div>`; }
function voiceConsolePage(data) { return `<div class="canonical-live-voice">${canonicalTopCards(data)}<div class="canonical-main"><div class="canonical-left">${canonicalConversation(data)}${panel("Rolling audio-event timeline",sharedVoiceFeed(data,true),{span:12})}</div><div class="canonical-right">${canonicalScan(data)}${canonicalDiagnostics(data)}${panel("Expanded diagnostics",`<details class="json-details"><summary>Show transcription JSON</summary><div class="json-viewer"><pre>${esc(JSON.stringify(data.speech_scan||{},null,2))}</pre></div><button class="button small" data-prototype="Copy transcription JSON">Copy JSON</button><button class="button small" data-prototype="Download transcription JSON">Download JSON</button></details>`,{span:12})}</div></div></div>`; }
