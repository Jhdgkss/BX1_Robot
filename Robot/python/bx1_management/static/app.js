...
function applyCanonicalTheme(id, persist = true) { const theme = CANONICAL_THEMES[id] || CANONICAL_THEMES.darkBlue; Object.entries(theme.vars).forEach(([key,value]) => document.documentElement.style.setProperty(key,value)); document.documentElement.dataset.theme = id; if (persist) localStorage.setItem("bx1-os-theme", id); }
function canonicalTopCards(data) { const bridge=data.audio_bridge||{}, a=bridge.audio||{}, r=bridge.recognition||{}, brain=data.voice?.brain?.state||"unavailable"; const rms=Number.isFinite(Number(a.rms_dbfs))?`${Number(a.rms_dbfs).toFixed(1)} dBFS`:"unavailable"; const gate=a.speaker_playback_active?"closed · speaker active":Number(a.echo_tail_remaining_s||0)>0?"closed · echo tail":a.gate_open===false?"closed":"open"; const engine=r.handoff?.engine_selected||r.engine||"unavailable"; const wake=data.voice?.voice_settings?.effective?.wake_phrases||["configured wake phrases"]; return `<div class="canonical-top-cards"><section class="canonical-card"><h4>Microphone</h4><div class="level-meter"><i style="width:${Math.max(0,Math.min(100,(Number(a.rms_dbfs)+90)/.9||0))}%"></i></div><strong>${esc(rms)}</strong><span>${esc(a.available===false?"unavailable":a.state||"stale")}</span><button class="button danger" data-audio-control="mic-mute">Mute Microphone</button><button class="button" data-audio-control="ptt">Push to Talk</button></section><section class="canonical-card"><h4>Listening / Wake Word</h4><div class="waveform"><i></i><i></i><i></i><i></i><i></i></div><strong>${esc(a.pipeline_state||a.state||"unavailable")}</strong><span>${esc(wake.join(", "))}</span><button class="button" data-audio-control="pause">Pause Listening</button></section><section class="canonical-card"><h4>Speaker</h4><strong>${a.speaker_playback_active?"active":"idle"}</strong><span>Volume ${esc(data.audio?.output?.volume??"unavailable")}</span><input type="range" min="0" max="100" value="80" aria-label="Speaker volume"><button class="button danger" data-audio-control="speaker-mute">Mute Speaker</button><button class="button" data-audio-control="stop">Stop Speaking</button></section><section class="canonical-card"><h4>Voice / System State</h4><strong>Gate ${esc(gate)}</strong><span>Brain ${esc(brain)}</span><span>STT ${esc(engine)}</span><span>${esc(a.timestamp?formatTimestamp(a.timestamp):"stale")}</span></section></div>`; }
...
function dashboardPage(data) { return `<div class="canonical-dashboard">${canonicalTopCards(data)}<div class="canonical-main"><div class="canonical-left">${canonicalConversation(data)}</div><div class="canonical-right">${canonicalScan(data)}${canonicalDiagnostics(data)}${canonicalQuick()}</div></div><details class="system-overview"><summary>System Overview</summary>${panel("BX1 OS components",dataList([["Management version",data.interface.version],["Robot Body version",data.robot_body?.version||"unavailable"],["Brain version",data.brain?.version||"unavailable"],["Brain connection",data.brain?.status||"unavailable"]]),{span:12})}</details></div>`; }
function voiceConsolePage(data) { return `<div class="canonical-live-voice">${canonicalTopCards(data)}<div class="canonical-main"><div class="canonical-left">${canonicalConversation(data)}${panel("Rolling audio-event timeline",sharedVoiceFeed(data,true),{span:12})}</div><div class="canonical-right">${canonicalScan(data)}${canonicalDiagnostics(data)}${canonicalQuick()}${panel("Expanded diagnostics",`<pre>${esc(JSON.stringify(data.speech_scan||{},null,2))}</pre><button class="button" data-prototype="Download diagnostic package">Download diagnostic package</button>`,{span:12})}</div></div></div>`; }
